...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    print(fname)
    df = pd.read_pickle(fname).sort_values(["user_id", "timestamp"]).sort_values(["user_id", "timestamp"])
    ...
    for column in ["content_id", "user_id", "content_type_id", "prior_question_had_explanation", "part",
        ...
        feature_factory_dict[column] = {
            ...
            "TargetEncoder": TargetEncoder(column=column, is_partial_fit=is_partial_fit, initial_score=0.66, initial_weight=200)
        }
