...
class SAKTDataset(Dataset):
    def __init__(self, group, n_skill, n_part=8, max_seq=100, is_test=False):
        ...
        for user_id in group.index:
            q, part, qa, tsdiff, elptime, is_val = group[user_id]
            if not is_test:
    ...
    def __getitem__(self, index):
        ...
        q_, part_,tsdiff_, elptime_, qa_,  _ = self.samples[user_id]
        ...
        if not self.is_test:
            ...
            qa_ = qa_[start:end]
            tsdiff_ = tsdiff_[start:end]
            elptime_ = elptime_[start:end]
            seq_len = len(q_)

        q = np.zeros(self.max_seq, dtype=int)
        part = np.zeros(self.max_seq, dtype=int)
        qa = np.zeros(self.max_seq, dtype=int)
        tsdiff = np.zeros(self.max_seq, dtype=int)
        elptime = np.zeros(self.max_seq, dtype=int)
        if seq_len >= self.max_seq:
            q[:] = q_[-self.max_seq:]
            part[:] = part_[-self.max_seq:]
            qa[:] = qa_[-self.max_seq:]
            tsdiff[:] = tsdiff_[-self.max_seq:]
            elptime[:] = elptime_[-self.max_seq:]
        else:
            q[-seq_len:] = q_
            part[-seq_len:] = part_
            qa[-seq_len:] = qa_
            tsdiff[-seq_len:] = tsdiff_
            elptime[-seq_len:] = elptime_
        ...
        return x, target_id, part, tsdiff, elptime, label
...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128):
        ...
        self.pos_encoder = PositionalEncoding(embed_dim, max_len=max_seq-1)
    ...
    def forward(self, x, question_ids, parts, tsdiff, elptime):
        device = x.device
        x = self.embedding(x)

        pos_id1 = torch.arange(x.size(1)).unsqueeze(1).to(device)
        p = self.part_embedding(parts)
        pos_x = self.pos_encoder(pos_id1)
        x = x + pos_x + p

        e = self.e_embedding(question_ids)
        t = self.tsdiff_embedding(tsdiff)
        pos_id2 = (torch.arange(x.size(1))+1).unsqueeze(1).to(device)
        elp = self.elptime_embedding(elptime)
        e = e + t + elp + pos_id2
...
def train_epoch(model, train_iterator, val_iterator, optim, criterion, device="cuda"):
    ...
    for item in tbar:
        ...
        tsdiff = item[3].to(device).long()
        elptime = item[4].to(device).long()
        label = item[5].to(device).float()

        optim.zero_grad()
        output, atten_weight = model(x, target_id, part, tsdiff, elptime)
    ...
    for d in tqdm(val_iterator):
        ...
        tsdiff = d[3].to(device).long()
        elptime = d[4].to(device).long()
        label = d[5].to(device).float()
...
def main(params: dict):
    ...
    df["timestamp_diff"] = (df["timestamp"] - df.groupby("user_id")["timestamp"].shift(1)) // 1000
    df["timestamp_diff"] = [x if x < 300 else 300 for x in df["timestamp_diff"].values]
    df["prior_question_elapsed_sec"] = [x//1000 if x//1000 < 300 else 300 for x in df["prior_question_elapsed_time"].values]

    df["is_val"] = 0
    df["is_val"].loc[val_idx] = 1
    df["group"] = (df.groupby("user_id")["user_id"].transform("count") - df.groupby("user_id").cumcount()) // max_len
    cols = ['user_id', 'content_id', 'answered_correctly', "group", "part", 'is_val', "timestamp_diff", "prior_question_elapsed_sec"]
    group = df[df["is_val"] == 0][cols].groupby(['user_id', "group"]).apply(lambda r: (
                r['content_id'].values,
                r['part'].values,
                r["timestamp_diff"].values,
                r["prior_question_elapsed_sec"].values,
                r['answered_correctly'].values,
                r["is_val"].values))
    dataset_train = SAKTDataset(group, 13523, max_seq=params["max_seq"])

    group = df[cols].groupby(['user_id']).apply(lambda r: (
                r['content_id'].values,
                r['part'].values,
                r["timestamp_diff"].values,
                r["prior_question_elapsed_sec"].values,
