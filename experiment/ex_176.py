...
def make_feature_factory_manager(split_num, model_id=None):
    ...
    feature_factory_dict["user_id"]["QuestionLectureTableEncoder2"] = QuestionLectureTableEncoder2(model_id=model_id,
                                                                                                   ...
                                                                                                   past_n=100,
                                                                                                   min_size=300)
    feature_factory_dict["user_id"]["QuestionQuestionTableEncoder2"] = QuestionQuestionTableEncoder3(model_id=model_id,
                                                                                                     is_debug=is_debug,
                                                                                                     past_n=100,
                                                                                                     min_size=300)
...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    for t in [1000, 3000]:
        df[f"elapsed_{t}"] = (df["prior_question_elapsed_time"] <= t).astype("int")
        df[f"elapsed_{t}"] = df.groupby("user_id")[f"elapsed_{t}"].cumsum()
    print(df.columns)
    print(df.shape)
