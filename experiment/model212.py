...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        embed_dim_cat_all = 16*9 + embed_dim
        ...
        self.prev_ans_idx_embedding = nn.Embedding(302, 16)
        self.prev_ans_content_id_embedding = nn.Embedding(4, 16)
...
def main(params: dict,
    ...
    os.makedirs("../input/feature_engineering/model212", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model212/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model212/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model212/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model212/val.pickle", "rb") as f:
