...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        self.prior_content_embedding = nn.Sequential(
            nn.Linear(self.embed_dim_cat, 32),
            nn.LayerNorm(32)
        )
    ...
    def forward(self, x, question_ids, parts, elapsed_time, duration_previous_content, prior_q, user_answer,
                rate_diff, container_id, prev_ans_idx, prev_ans_content_id, prior_content_id):
        device = x.device
        ...
        x = torch.cat([x, el_time_emb, dur_emb, e, p, prior_q_emb, container_emb,
                       ...
                       prior_content_user_answer_emb], dim=2)
...
def main(params: dict,
    ...
    os.makedirs("../input/feature_engineering/model213", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model213/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model213/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model213/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model213/val.pickle", "rb") as f:
