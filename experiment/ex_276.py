...
def make_feature_factory_manager(split_num, model_id=None):
    ...
    feature_factory_dict["user_id"]["Past1ContentTypeId"] = PastNFeatureEncoder(column="content_type_id",
                                                                                ...
                                                                                agg_funcs=["mean"],
                                                                                remove_now=False)
    feature_factory_dict["user_id"]["PastUserAnswer"] = PastNFeatureEncoder(column="user_answer",
                                                                            past_ns=[5, 10, 20],
                                                                            agg_funcs=["nunique"],
                                                                            remove_now=True)
...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    df["from_last_lecture"] = [x[0] if not x[1] == -1 else np.nan for x in
                               df[["from_last_lecture", "last_lecture"]].values]
