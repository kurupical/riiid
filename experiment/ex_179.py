...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    df["diff1"] = df["qq_table2_last"] - df["prev_ans_te"]
    df["diff1sum"] = df.groupby("user_id")["diff1"].cumsum()
    df["diff1mean"] = df.groupby("user_id")["diff1"].cumsum() / (df.groupby("user_id")["diff1"].cumcount()+1)
    print(df.columns)
    print(df.shape)
