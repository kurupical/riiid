...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        diff_timedelta_groupby_user_id_part_bin500_ = self.samples[user_id]["diff_timedelta_groupby_user_id_part_bin500"]
        ...
        if not self.is_test:
            ...
            diff_timedelta_groupby_user_id_part_bin500_ = diff_timedelta_groupby_user_id_part_bin500_[start:end]
            seq_len = len(q_)
        ...
        diff_timedelta_groupby_user_id_part_bin500 = np.zeros(self.max_seq, dtype=int)
        if seq_len >= self.max_seq:
            ...
            diff_timedelta_groupby_user_id_part_bin500[:] = diff_timedelta_groupby_user_id_part_bin500_[-self.max_seq:]
        else:
            q[-seq_len:] = q_
            part[-seq_len:] = part_
            qa[-seq_len:] = qa_
            ...
            diff_timedelta_groupby_user_id_part_bin500[-seq_len:] = diff_timedelta_groupby_user_id_part_bin500_
        ...
        diff_timedelta_groupby_user_id_part_bin500 = diff_timedelta_groupby_user_id_part_bin500[1:]
        ...
        return {
            ...
            "diff_timedelta_groupby_user_id_part_bin500": diff_timedelta_groupby_user_id_part_bin500
        }
...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        embed_dim_cat_all = 32*10 + embed_dim
        ...
        self.diff_timedelta_groupby_user_id_part_bin500_embedding = nn.Embedding(502, 32)
    ...
    def forward(self, x, question_ids, parts, elapsed_time, duration_previous_content, prior_q, user_answer,
                rate_diff, container_id, prev_ans_idx, prev_ans_content_id, diff_timedelta_groupby_user_id_part_bin500):
        device = x.device
        ...
        diff_timedelta_groupby_user_id_part_bin500_emb = self.diff_timedelta_groupby_user_id_part_bin500_embedding(diff_timedelta_groupby_user_id_part_bin500)
        x = torch.cat([x, el_time_emb, dur_emb, e, p, prior_q_emb, user_answer_emb, container_emb,
                       prev_ans_idx_emb, prev_ans_content_id_emb,
                       diff_timedelta_groupby_user_id_part_bin500_emb], dim=2)
...
def train_epoch(model, train_iterator, val_iterator, optim, criterion, scheduler, epoch, device="cuda"):
    ...
    for item in tbar:
        ...
        diff_timedelta_groupby_user_id_part_bin500 = item["diff_timedelta_groupby_user_id_part_bin500"].to(device).long()
        ...
        output = model(x, target_id, part, elapsed_time,
                       ...
                       diff_timedelta_groupby_user_id_part_bin500)
...
def main(params: dict,
    ...
    column_config = {
        ...
        "diff_timedelta_groupby_user_id_part_bin500": {"type": "category"}
    }
    ...
    if not load_pickle or is_debug:
        ...
        feature_factory_dict["user_id"]["PreviousAnswer2"] = PreviousAnswer2(groupby="user_id",
                                                                             ...
                                                                             model_id=model_id,
                                                                             n=300)
        feature_factory_dict["user_id"]["MeanAggregatorShiftDiffTimeElapsedTimeByUserIdPart"] = \
            MeanAggregator(column=["user_id", "part"],
                           agg_column="duration_previous_content",
                           remove_now=False)
        ...
        df["diff_timedelta_groupby_user_id_part_bin500"] = \
            [f(x) for x in df["diff_mean_duration_previous_content_by_['user_id', 'part']"].values]
        ...
        print(df["diff_timedelta_groupby_user_id_part_bin500"].value_counts())
    ...
    os.makedirs("../input/feature_engineering/model215_2", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model215_2/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model215_2/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model215_2/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model215_2/val.pickle", "rb") as f:
    ...
    if is_make_feature_factory:
        ...
        feature_factory_dict["user_id"]["ElapsedTimeBinningEncoder"] = ElapsedTimeBinningEncoder()
        feature_factory_dict["user_id"]["MeanAggregatorShiftDiffTimeElapsedTimeByUserIdPart"] = \
            MeanAggregator(columns=["user_id", "part"],
