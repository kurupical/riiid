...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        user_answer = ua[:-1] + 1 # 0: nan, 1: lecture, 2~5: answer
        ...
        label_answered_correctly = label[1:]
        label_user_answer = ua[1:] + 1
        label_user_answer = np.identity(6)[label_user_answer] # one-hot-encode
        ...
        return {
            ...
            "label_answered_correctly": label_answered_correctly,
            "label_user_answer": label_user_answer,
            "prior_q": prior_q,
            "rate_diff": rate_diff,
...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        self.ffn_answered_correctly = FFN(embed_dim_all)
        self.ffn_user_answer = FFN(embed_dim_all)
        self.dropout = nn.Dropout(dropout/2)
        self.pred_user_answer = nn.Linear(embed_dim_all, 6) # 本当は4だけど、lectureの場合とseq_lenがnullの場合を実装するのが面倒だったので
        self.linear_user_answer = nn.Linear(6, embed_dim_all)
        self.pred_answered_correctly = nn.Sequential(
            nn.Linear(embed_dim_all*2, embed_dim_all//2),
            nn.LayerNorm(embed_dim_all//2),
            nn.Dropout(dropout),
            nn.Linear(embed_dim_all//2, 1)
        )
    ...
    def forward(self, x, question_ids, parts, elapsed_time, duration_previous_content, prior_q, user_answer,
        ...
        x_user_answer = self.ffn_user_answer(x) + att_dec
        x_user_answer = self.dropout(x_user_answer)
        y_user_answer = self.pred_user_answer(x_user_answer)

        x_answered_correctly = self.ffn_answered_correctly(x) + att_dec
        x_answered_correctly = self.dropout(x_answered_correctly)
        x_final = torch.cat([x_answered_correctly,
                             self.linear_user_answer(y_user_answer)], dim=2)
        y_answered_correctly = self.pred_answered_correctly(x_final)

        return y_answered_correctly.squeeze(-1), y_user_answer.squeeze(-1)
...
def train_epoch(model, train_iterator, val_iterator, optim, criterion, scheduler, epoch, device="cuda"):
    ...
    for item in tbar:
        ...
        label_answered_correctly = item["label_answered_correctly"].to(device).float()
        label_user_answer = item["label_user_answer"].to(device).float()
        ...
        output_answered_correctly, output_user_answer = model(
            x, target_id, part, elapsed_time,
        ...
        target_idx = (label_answered_correctly.view(-1) >= 0).nonzero()
        loss1 = criterion(output_answered_correctly.view(-1)[target_idx], label_answered_correctly.view(-1)[target_idx])
        loss2 = criterion(output_user_answer.view(-1)[target_idx], label_user_answer.view(-1)[target_idx])
        loss = loss1 + loss2
        loss.backward()
        optim.step()
        scheduler.step()
        ...
        output = output_answered_correctly[:, -1]
        label = label_answered_correctly[:, -1]
        ...
        tbar.set_description('[loss]answered_correctly: {:.4f}, user_answer: {:.4f}'.format(loss1, loss2))
    ...
    with torch.no_grad():
        for item in tqdm(val_iterator):
            ...
            output, _ = model(x, target_id, part, elapsed_time,
                              duration_previous_content, prior_question_had_explanation, user_answer,
                              rate_diff, container_id, prev_ans_idx, prev_answer_content_id)
            preds.extend(torch.nn.Sigmoid()(output[:, -1]).view(-1).data.cpu().numpy().tolist())
            labels.extend(label_answered_correctly[:, -1].view(-1).data.cpu().numpy())
            i += 1
...
def main(params: dict,
    ...
    if not load_pickle or is_debug:
        ...
        df = df[["user_id", "content_id", "content_type_id", "part", "user_answer", "answered_correctly",
                 ...
                 "previous_answer_index_content_id", "previous_answer_content_id"]]
        print(df.head(10))
    ...
    os.makedirs("../input/feature_engineering/model174", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model174/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model174/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model174/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model174/val.pickle", "rb") as f:
    ...
    with torch.no_grad():
        for item in tqdm(dataloader_val):
            ...
            preds.extend(torch.nn.Sigmoid()(output[:, -1]).view(-1).data.cpu().numpy().tolist())
            labels.extend(label_answered_correctly[:, -1].view(-1).data.cpu().numpy().tolist())
