...
def make_feature_factory_manager(split_num, model_id=None):
    ...
    feature_factory_dict[column][f"MeanAggregatorUserRateContentIdByUserId"] = MeanAggregator(column=["prev_user_answer", "prev_content_id"],
                                                                                              agg_column="user_id_rating",
                                                                                              remove_now=False)
...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    df["prev_user_answer"] = df.groupby("user_id")["user_answer"].shift(1).fillna(-1)
    ...
    df = df.drop(["prev_user_answer", "prev_content_id"], axis=1)
