...
def make_feature_factory_manager(split_num, model_id=None):
    ...
    for column in ["user_id", "content_id", ("user_id", "kmeans_tags10")]:
...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    df_question["kmeans_tags10"] = KMeans(n_clusters=10).fit_predict(df_question[target_cols].values)

    df = pd.merge(df, df_question[["question_id", "kmeans_tags10"]], how="left")
