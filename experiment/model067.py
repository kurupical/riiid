...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2):
        ...
        self.n_skill = n_skill
        self.embed_dim = 256+16

        self.embedding = nn.Embedding(3, 16)
        self.e_embedding = nn.Embedding(n_skill + 1, 256)
        self.pos_embedding_enc = nn.Embedding(max_seq - 1, 256+16)

        self.pos_embedding_dec = nn.Embedding(max_seq - 1, 256+16)
        self.part_embedding = nn.Embedding(8, 16)
        self.elapsed_time_embedding = nn.Embedding(302, 128)
        self.duration_previous_content_embedding = nn.Embedding(302, 128)
