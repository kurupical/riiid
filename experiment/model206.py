...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        lag_time_ = self.samples[user_id]["lag_time"]
        lag_time_ = np.clip(np.array(lag_time_)//1000, 1, 300)
...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        self.lag_time_embedding = nn.Embedding(302, 32)
        ...
        self.gru1 = nn.GRU(input_size=embed_dim_all, hidden_size=embed_dim_all//2)
        self.layer_normal_gru1 = nn.LayerNorm(embed_dim_all//2)
        self.gru2 = nn.GRU(input_size=embed_dim_all//2, hidden_size=embed_dim_all//4)

        self.layer_normal = nn.LayerNorm(embed_dim_all//4)

        self.ffn = FFN(embed_dim_all//4)
        self.dropout = nn.Dropout(dropout/2)
        self.pred = nn.Linear(embed_dim_all//4, 1)
    ...
    def forward(self, item, device):
        ...
        lag_time = item["lag_time"].to(device).long()
        ...
        x = torch.cat([x, el_time_emb, dur_emb, e, p, prior_q_emb, container_emb,
                       content_id_delta_emb, last_content_id_acc_emb,
                       lag_time_emb], dim=2)
        ...
        att_dec = self.transformer_enc(x,
                                       mask=att_mask)
        att_dec, _ = self.gru1(att_dec)
        att_dec = self.layer_normal_gru1(att_dec)
        att_dec, _ = self.gru2(att_dec)
...
def main(params: dict,
    ...
    # model206
    df["lag_time"] = df["timestamp_delta"] - df.groupby("user_id")["prior_question_elapsed_time"].shift(-1)
    ...
    os.makedirs("../input/feature_engineering/model206", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model206/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model206/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model206/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model206/val.pickle", "rb") as f:
...
if __name__ == "__main__":
    ...
    for dropout in [0.5, 0.3, 0.2, 0.1]:
        lr = 1e-3
        cont_emb = 16
        cat_emb = 256
        if is_debug:
