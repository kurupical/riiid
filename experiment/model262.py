...
class SAKTModel(nn.Module):
    ...
    def forward(self, item, device):
        ...
        weight = duration_previous_content_row.cumsum(dim=1)
        weight = torch.clip(weight, 60*60*24*1000*3, np.inf)  # 3日以下はweight全部1
        weight = weight / weight[:, -1].view(-1, 1)  # [bs, s_len]
        weight = torch.clip(weight, 0.1, 1)

        att_dec = att_dec * weight.unsqueeze(2)
...
def main(params: dict,
    ...
    if not load_pickle or is_debug:
        ...
        df = df[["user_id", "content_id", "content_type_id", "part", "user_answer", "answered_correctly",
                 ...
                 "timediff-elapsedtime_bin500", "duration_previous_content"]]
        print(df.head(10))
    ...
    os.makedirs("../input/feature_engineering/model261", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model261/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model261/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model261/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model261/val.pickle", "rb") as f:
