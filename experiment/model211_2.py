...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        timediff_elapsedtime_ = self.samples[user_id]["timediff-elapsedtime_bin500"]
...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        self.timediff_elapsedtime_embedding = nn.Embedding(502, 32)
...
def main(params: dict,
    ...
    if not load_pickle or is_debug:
        ...
        print(df.head(10))
        print(df["timediff-elapsedtime_bin500"].value_counts())
    ...
    os.makedirs("../input/feature_engineering/model211-2", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model211-2/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model211-2/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model211-2/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model211-2/val.pickle", "rb") as f:
