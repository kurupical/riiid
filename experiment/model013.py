...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        q_ = self.samples[user_id]["content_id"]
...
def main(params: dict):
    ...
    df = df[["user_id", "content_id", "part", "answered_correctly"]]
    ...
    df["w_user_id"] = df["user_id"]
    df["user_id"] = df["user_id"].astype(str) + "_" + df["group"].astype(str)
    logger = get_logger()
    ff_for_transformer = FeatureFactoryForTransformer(column_config={"content_id": {"type": "category"},
                                                                     "part": {"type": "category"}},
    ...
    print(group)
    dataset_train = SAKTDataset(group, 13782, max_seq=params["max_seq"])

    df["user_id"] = df["w_user_id"]
    ff_for_transformer = FeatureFactoryForTransformer(column_config={"content_id": {"type": "category"},
                                                                     "part": {"type": "category"}},
                                                      dict_path="../feature_engineering/",
                                                      sequence_length=params["max_seq"],
    ...
    group = ff_for_transformer.all_predict(df)
    dataset_val = SAKTDataset(group, 13782, is_test=True, max_seq=params["max_seq"])
    ...
    model = SAKTModel(13782, embed_dim=params["embed_dim"], max_seq=params["max_seq"])
