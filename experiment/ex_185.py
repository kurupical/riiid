...
def make_feature_factory_manager(split_num, model_id=None):
    ...
    feature_factory_dict["user_id"]["Past1ContentTypeId"] = PastNFeatureEncoder(column="content_type_id",
                                                                                ...
                                                                                agg_funcs=["mean"],
                                                                                remove_now=False)
    feature_factory_dict["user_id"]["Past1AnswerCountRatio"] = PastNFeatureEncoder(column="userans_count_ratio",
                                                                                   past_ns=[10],
                                                                                   agg_funcs=["mean", "min"],
                                                                                   remove_now=False)
...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    df["userans_count_ratio"] = \
        df.groupby(["content_id", "user_answer"])["content_id"].transform(np.size) / df.groupby(["content_id"])["content_id"].transform(np.size)
    df["userans_count_ratio"] = df.groupby("user_id")["userans_count_ratio"].shift(1)
