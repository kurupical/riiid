...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {
            ...
            "elapsed_time": get_data(key="elapsed_time_bin300", remove_now=True),
            "label_elapsed_time": get_data(key="elapsed_time_bin300"),
...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        self.ffn_elapsed_time = nn.Sequential(
            nn.Linear(embed_dim_all, embed_dim_all//2),
            nn.LayerNorm(embed_dim_all//2),
            nn.Linear(embed_dim_all//2, embed_dim_all//4),
            nn.LayerNorm(embed_dim_all//4),
            nn.Dropout(dropout/2),
        )

        self.pred_elapsed_time = nn.Linear(embed_dim_all//4, 1)

        self.ffn = FFN(embed_dim_all)
        self.dropout = nn.Dropout(dropout/2)
        self.pred_target = nn.Linear(embed_dim_all+embed_dim_all//4, 1)
    ...
    def forward(self, item, device):
        ...
        pred_elapsed_time = self.ffn_elapsed_time(x)
        x = self.ffn(x) + att_dec
        x = torch.cat([x, pred_elapsed_time], dim=2)
        x = self.dropout(x)
        x = self.pred_target(x)

        pred_elapsed_time = self.pred_elapsed_time(pred_elapsed_time)

        return x.squeeze(-1), pred_elapsed_time.squeeze(-1)
...
def train_epoch(model, train_iterator, val_iterator, optim, criterion, scheduler, epoch, device="cuda"):
    ...
    for item in tbar:
        optim.zero_grad()
        label = item["label"].to(device).float()
        label_elapsed_time = item["label_elapsed_time"].to(device).float()
        output, output_elapsed_time = model(item, device)

        target_idx = (label.view(-1) >= 0).nonzero()
        loss1 = criterion(output.view(-1)[target_idx], label.view(-1)[target_idx])
        loss2 = nn.MSELoss()(output_elapsed_time.view(-1)[target_idx]/10, label_elapsed_time.view(-1)[target_idx]/10)
        loss = loss1 + loss2
        loss.backward()
        optim.step()
        scheduler.step()
...
def main(params: dict,
    ...
    column_config = {
        ...
        "part": {"type": "category"},
        "elapsed_time_bin300": {"type": "category"},
    ...
    if not load_pickle or is_debug:
        ...
        w_df = df[["user_id", "task_container_id", "prior_question_elapsed_time"]].drop_duplicates(["user_id", "task_container_id"])
        w_df["elapsed_time"] = w_df.groupby(["user_id", "task_container_id"])["prior_question_elapsed_time"].shift(-1).fillna(0)
        w_df["elapsed_time_bin300"] = [x if x < 300 else 300 for x in w_df["elapsed_time"].values]
        df = pd.merge(df, w_df, how="inner")
        print(len(df))
        ...
        df = df[["user_id", "content_id", "content_type_id", "part", "user_answer", "answered_correctly",
                 ...
                 "timediff-elapsedtime_bin500", "elapsed_time_bin300"]]
        print(df.head(10))
    ...
    os.makedirs("../input/feature_engineering/model273", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model273/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model273/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model273/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model273/val.pickle", "rb") as f:
