...
df["group"] = (df.groupby("user_id")["user_id"].transform("count") - df.groupby("user_id").cumcount()) // max_seq
print(df["group"].value_counts())

print(len(train_idx))
print(len(val_idx))
group = df[['user_id', 'content_id', 'answered_correctly', "group", 'is_val']].groupby(['user_id', "group"]).apply(lambda r: (
            r['content_id'].values,
            r['answered_correctly'].values,
            r["is_val"].values))
dataset_train = SAKTDataset(group, 13523, max_seq=max_seq)

group = df[['user_id', 'content_id', 'answered_correctly', 'is_val']].groupby(['user_id']).apply(lambda r: (
            r['content_id'].values,
            r['answered_correctly'].values,
            r["is_val"].values))
dataset_val = SAKTDataset(group, 13523, is_test=True, max_seq=max_seq)
...
print("single transformer: {:.4f}".format(roc_auc_score(df_oof2["target"].values, df_oof2["predict"].values)))
print("lgbm: {:.4f}".format(roc_auc_score(df_oof2["target"].values, df_oof2["predict_lgbm"].values)))
