...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return x, target_id, part, label[-1]
...
class SAKTModel(nn.Module):
    ...
    def forward(self, x, question_ids, parts):
        ...
        enc = self.layer_normal(enc + e.permute(1, 0, 2))
        ...
        att_self_dec = att_self_dec.permute(1, 0, 2)
        att_output, att_weight = self.multi_att_dec(att_self_dec, att_self_dec, x, attn_mask=att_mask)
        att_output = self.layer_normal(att_output + enc)
        att_output = torch.mean(att_output, dim=0)  # get newest data
