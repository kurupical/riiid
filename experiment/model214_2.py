...
def main(params: dict,
    ...
    if not load_pickle or is_debug:
        ...
        df["diff_timedelta_groupby_user_id_part"] = \
            [f(x) for x in df["diff_mean_duration_previous_content_cap100k_by_['user_id', 'part']"]]
    ...
    os.makedirs("../input/feature_engineering/model214", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model214/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model214/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model214/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model214/val.pickle", "rb") as f:
