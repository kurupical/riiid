from feature_engineering.feature_factory import \
    ...
    PreviousNSameContentIdAnsweredCorrectly
...
def make_feature_factory_manager(split_num, model_id=None):
    ...
    feature_factory_dict["user_id"]["PreviousNSameContentIdAnsweredCorrectly"] = PreviousNSameContentIdAnsweredCorrectly(n=5)
    feature_factory_dict["previous_5_same_content_id_ans"] = {
        "TargetEncoder": TargetEncoder(column="previous_5_same_content_id_ans")
    }
...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    print(df.columns)
    df["sum_enc_user_id"] = df["target_enc_user_id"] * df["count_enc_user_id"]
    df["sum_enc_['user_id', 'part']"] = df["target_enc_['user_id', 'part']"] * df["count_enc_['user_id', 'part']"]
    ...
    df = df.drop(["user_answer", "tags", "type_of", "bundle_id", "previous_5_ans",
                  "tag", "content_type_id", "content_past3", "previous_5_same_content_id_ans"],
