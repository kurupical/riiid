...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        for item_name, idx in idx_dict.items():
            item_ary[idx, -data_length:] = self.samples[user_id][item_name][start:end]
...
def main(params: dict,
    ...
    if not load_pickle or is_debug:
        df_val_row = pd.read_feather("../../riiid_takoi/notebook/fe/validation_row_id.feather").head(len(df) // 10)
        if is_debug:
    ...
    os.makedirs("../input/feature_engineering/model208_all", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model208_all/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model208_all/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model208_all/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model208_all/val.pickle", "rb") as f:
