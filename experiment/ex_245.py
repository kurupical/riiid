...
def make_feature_factory_manager(split_num, model_id=None):
    ...
    for column in ["user_id", "content_id", "content_past3"]:
...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    df["content_past3"] = df["content_id"].astype(str)
    for i in [1, 2, 3]:
        df["content_past3"] += "_" + df.groupby("user_id")["content_id"].shift(i).astype(str) + "_" + df.groupby("user_id")["user_answer"].shift(i).astype(str)
    print(df["content_past3"].head(20))
