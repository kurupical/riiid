...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2):
        ...
        self.rate_diff_embedding = nn.Sequential(
            nn.LayerNorm(1),
    ...
    def forward(self, x, question_ids, parts, elapsed_time, duration_previous_content, prior_q, user_answer,
        ...
        rate_diff = self.rate_diff_embedding(rate_diff.view(x.size(0), x.size(1), -1))
        x = self.cat_embedding(x)
        x = torch.cat([x, rate_diff], dim=2)
...
def main(params: dict,
    ...
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model075/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model075/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model075/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model075/val.pickle", "rb") as f:
