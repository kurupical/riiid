...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        duration_previous_content_logscale_ = self.samples[user_id]["duration_previous_content_logscale"]
        ...
        if not self.is_test:
            ...
            duration_previous_content_logscale_ = duration_previous_content_logscale_[start:end]
            seq_len = len(q_)
        ...
        duration_previous_content_logscale = np.zeros(self.max_seq, dtype=int)
        if seq_len >= self.max_seq:
            ...
            duration_previous_content_logscale[:] = duration_previous_content_logscale_[-self.max_seq:]
        else:
            q[-seq_len:] = q_
            part[-seq_len:] = part_
            qa[-seq_len:] = qa_
            ...
            duration_previous_content_logscale[-seq_len:] = duration_previous_content_logscale_
        ...
        duration_previous_content_logscale = duration_previous_content_logscale[1:]
        ...
        return {
            ...
            "duration_previous_content_logscale": duration_previous_content_logscale,
            "label": label,
            "prior_q": prior_q,
            "rate_diff": rate_diff,
...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2):
        ...
        self.transformer_enc = nn.TransformerEncoder(encoder_layer=encoder_layer, num_layers=2)
    ...
    def forward(self, x, question_ids, parts, elapsed_time, duration_previous_content, prior_q, user_answer,
                rate_diff, qq_table_mean, qq_table_min, duration_previous_content_logscale):
        device = x.device
        ...
        cont = torch.cat([rate_diff, qq_table_mean, qq_table_min, duration_previous_content_logscale], dim=1)
...
def train_epoch(model, train_iterator, val_iterator, optim, criterion, scheduler, device="cuda"):
    ...
    for item in tbar:
        ...
        duration_previous_content_logscale = item["duration_previous_content_logscale"].to(device).float()
...
def main(params: dict,
    ...
    column_config = {
        ...
        "duration_previous_content_logscale": {"type": "numeric"}
    }
    ...
    if not load_pickle or is_debug:
        ...
        df["duration_previous_content_logscale"] = np.log(df["duration_previous_content"].fillna(1).values)
        df["duration_previous_content_logscale"] = df["duration_previous_content_logscale"].replace(-np.inf, 1).replace(-np.inf, 1)
        df = df[["user_id", "content_id", "content_type_id", "part", "user_answer", "answered_correctly",
                 ...
                 "qq_table2_mean", "qq_table2_min", "duration_previous_content_logscale"]].replace(-99, -1)
    ...
    os.makedirs("../input/feature_engineering/model089", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model089/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model089/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model089/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model089/val.pickle", "rb") as f:
