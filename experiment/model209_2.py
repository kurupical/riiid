...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        self.continuous_embedding = ContEmbedding(input_dim=3, embed_dim=cont_emb, seq_len=max_seq)
    ...
    def forward(self, item, device):
        ...
        content_id_rate = item["content_id_rate"].to(device).long()
        user_id_rate = item["user_id_rate"].to(device).long()
        ...
        cont = torch.cat([rate_diff, content_id_rate, user_id_rate], dim=1)
...
def main(params: dict,
    ...
    os.makedirs("../input/feature_engineering/model209", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model209/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model209/val.pickle", "wb") as f:
