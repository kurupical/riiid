...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    df["last_lecture"] = [x[0] if x[1] == 1 else np.nan for x in df[["last_lecture", "content_type_id"]].values]
    df["last_lecture"] = df["last_lecture"].fillna(method="ffill")
    df["from_last_lecture"] = df.groupby("user_id")["last_lecture"].transform("count")

    df["last_lecture_ts"] = df.groupby("user_id")["content_id"].shift(1)
    df["last_lecture_ts"] = [x[0] if x[1] == 1 else np.nan for x in df[["last_lecture_ts", "content_type_id"]].values]
    df["last_lecture_ts"] = df["timestamp"] - df["last_lecture_ts"].fillna(method="ffill")

    print(df["from_last_lecture"].value_counts())
...
for i, fname in enumerate(glob.glob("../input/riiid-test-answer-prediction/split10/*")):
    print(fname)
    model_id = os.path.basename(fname).replace(".pickle", "")
    data_types_dict = {
        'row_id': 'int64',
