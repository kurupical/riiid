...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    df["long_kyukei"] = (df["timestamp"] - df.groupby("user_id")["timestamp"].shift(1) >1000*60*60*24*30).astype("uint8")
    df["long_kyukei"] = df.groupby("user_id")["long_kyukei"].cumsum().astype("uint8")
