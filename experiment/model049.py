...
from feature_engineering.feature_factory import FeatureFactoryManager, DurationPreviousContent
...
def train_epoch(model, train_iterator, val_iterator, optim, criterion, device="cuda"):
    ...
    for item in tbar:
        ...
        optim.zero_grad()
        output, atten_weight = model(x, target_id, part, elapsed_time, duration_previous_content)
...
def main(params: dict):
    ...
    df = df[["user_id", "content_id", "content_type_id", "part", "user_answer", "answered_correctly", "prior_question_elapsed_time", "duration_previous_content"]]

    print("data preprocess")
    df["prior_question_elapsed_time_bin300"] = [x//1000 if x//1000 < 300 else 300 for x in df["prior_question_elapsed_time"].fillna(0).values]
    df["duration_previous_content_bin300"] = [x//1000 if x//1000 < 300 else 300 for x in df["duration_previous_content"].fillna(0).values]
    ...
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model048/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model048/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if load_pickle:
        with open(f"../input/feature_engineering/model048/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model048/val.pickle", "rb") as f:
            dataset_val = pickle.load(f)
    dataloader_train = DataLoader(dataset_train, batch_size=512, shuffle=True, num_workers=1)
    dataloader_val = DataLoader(dataset_val, batch_size=512, shuffle=False, num_workers=1)
    ...
    auc_transformer = roc_auc_score(df_oof["target"].values, df_oof["predict"].values)
