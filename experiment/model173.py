...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        emb1 = 8
        emb2 = 16
        embed_dim_cat_all = emb1*5 + emb2*4 + embed_dim
        embed_dim_all = embed_dim_cat_all + cont_emb

        self.embedding = nn.Embedding(4, emb1)
        self.user_answer_embedding = nn.Embedding(6, emb1)
        self.prior_question_had_explanation_embedding = nn.Embedding(4, emb1)
        self.e_embedding = nn.Embedding(n_skill + 1, self.embed_dim_cat)
        self.part_embedding = nn.Embedding(8, emb1)
        self.elapsed_time_embedding = nn.Embedding(302, emb2)
        self.duration_previous_content_embedding = nn.Embedding(302, emb2)
        self.container_embedding = nn.Embedding(302, emb2)
        self.prev_ans_idx_embedding = nn.Embedding(302, emb2)
        self.prev_ans_content_id_embedding = nn.Embedding(4, emb1)
...
def main(params: dict,
    ...
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model155/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model155/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model155/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model155/val.pickle", "rb") as f:
