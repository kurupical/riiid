...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    feature_factory_dict["tags"] = {
        "TagsSeparator": TagsSeparator2()
    }
    # tags = [f"tag_{x}" for x in np.arange(1, 188)]
    tags = [f"tag_{x}" for x in np.arange(1, 50)]
    ...
    for column in ["part", "content_id", "content_type_id", "user_id", "prior_question_had_explanation",
                   ...
                   ("user_id", "part1"), ("user_id", "part2"), ("user_id", "part3"), ("user_id", "part4"),
                   ("user_id", "part5"), ("user_id", "part6"), ("user_id", "part7")]:
    ...
    feature_factory_dict["part"]["PartSeparator"] = PartSeparator()
    ...
    for column in ["user_id", "content_id"]:
        feature_factory_dict[column][f"MeanAggregatorPriorQuestionElapsedTimeby{column}"] = MeanAggregator(column=column,
                                                                                                           ...
                                                                                                           remove_now=True)
        feature_factory_dict["user_id"][f"MeanAggregatorPriorQuestionshiftdiffbyuser_id"] = MeanAggregator(column=column,
                                                                                                           agg_column="shiftdiff_timestamp_by_user_id",
                                                                                                           remove_now=False)
    for column in ["part1", "part2", "part3", "part4", "part5", "part6", "part7"]:
        feature_factory_dict[column] = {}
        feature_factory_dict[column][f"MeanAggregatorPriorQuestionElapsedTimeby{column}"] = MeanAggregator(column=column,
                                                                                                           agg_column="prior_question_elapsed_time",
                                                                                                           remove_now=True)
        feature_factory_dict[column][f"MeanAggregatorPriorQuestionshiftdiffby{column}"] = MeanAggregator(column=column,
                                                                                                         agg_column="shiftdiff_timestamp_by_user_id",
                                                                                                         remove_now=False)
    ...
    del_col = [f"tag_{x}" for x in np.arange(1, 188)] + [f"count_enc_['user_id', 'tag_{x}']" for x in np.arange(1, 50)]
    print(df.shape)
