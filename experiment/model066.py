...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        x = qa[:-1].copy() + 1 # 0: lecture 1: not correct 2: correct
...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2):
        ...
        self.pos_embedding_enc = nn.Embedding(max_seq - 1, 192)
        self.pos_embedding_dec = nn.Embedding(max_seq - 1, 192)
        self.e_embedding = nn.Embedding(n_skill + 1, 192-16)
        self.part_embedding = nn.Embedding(8, 16)
        self.elapsed_time_embedding = nn.Embedding(302, 64)
        self.duration_previous_content_embedding = nn.Embedding(302, 64)
        encoder_layer = nn.TransformerEncoderLayer(d_model=192, nhead=num_heads)
        self.transformer_enc = nn.TransformerEncoder(encoder_layer=encoder_layer, num_layers=2)
        decoder_layer = nn.TransformerDecoderLayer(d_model=192, nhead=num_heads)
        self.transformer_dec = nn.TransformerDecoder(decoder_layer=decoder_layer, num_layers=2)

        self.dropout = nn.Dropout(0.2)
        self.layer_normal = nn.LayerNorm(192)

        self.ffn = FFN(192)
        self.pred = nn.Linear(192, 1)
    ...
    def forward(self, x, question_ids, parts, elapsed_time, duration_previous_content):
        ...
        x = torch.cat([x, el_time_emb, dur_emb], dim=2)
        x += pos_x
...
def main(params: dict,
    ...
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model064/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model064/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model064/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model064/val.pickle", "rb") as f:
