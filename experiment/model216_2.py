...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        user_id_rate_ = self.samples[user_id]["user_id_rating"]
        content_id_rate_ = self.samples[user_id]["content_rating"]

        # Z-value
        rate_diff_ = (rate_diff_ + 138.4) / 196.07
        user_id_rate_ = (user_id_rate_ - 1538.29) / 107.34
        content_id_rate_ = (content_id_rate_ - 1397.126) / 178.42
        ...
        if not self.is_test:
            ...
            user_id_rate_ = user_id_rate_[start:end]
            content_id_rate_ = content_id_rate_[start:end]
            seq_len = len(q_)
        ...
        user_id_rate = np.zeros(self.max_seq, dtype=int)
        content_id_rate = np.zeros(self.max_seq, dtype=int)
        if seq_len >= self.max_seq:
            ...
            user_id_rate[:] = user_id_rate_[-self.max_seq:]
            content_id_rate[:] = content_id_rate_[-self.max_seq:]
        else:
            q[-seq_len:] = q_
            part[-seq_len:] = part_
            qa[-seq_len:] = qa_
            ...
            user_id_rate[-seq_len:] = user_id_rate_
            content_id_rate[-seq_len:] = content_id_rate_
        ...
        user_id_rate = user_id_rate[1:]
        content_id_rate = content_id_rate[1:]
        ...
        return {
            ...
            "user_id_rate": user_id_rate,
            "content_id_rate": content_id_rate
        }
...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        embed_dim_all = embed_dim_cat_all + cont_emb*3
        ...
        self.continuous_embedding_ratediff = ContEmbedding(input_dim=1, embed_dim=cont_emb, seq_len=max_seq)
        self.continuous_embedding_user_id_rate = ContEmbedding(input_dim=1, embed_dim=cont_emb, seq_len=max_seq)
        self.continuous_embedding_content_id_rate = ContEmbedding(input_dim=1, embed_dim=cont_emb, seq_len=max_seq)
    ...
    def forward(self, item, device):
        ...
        prev_ans_content_id = item["previous_answer_content_id"].to(device).long()
        content_id_rate = item["content_id_rate"].to(device).float()
        user_id_rate = item["user_id_rate"].to(device).float()
        ...
        rate_diff_emb = self.continuous_embedding_ratediff(rate_diff.view(x.size(0), x.size(1), -1))
        content_id_rate_emb = self.continuous_embedding_content_id_rate(content_id_rate.view(x.size(0), x.size(1), -1))
        user_id_rate_emb = self.continuous_embedding_user_id_rate(user_id_rate.view(x.size(0), x.size(1), -1))

        x = self.cat_embedding(x)
        x = torch.cat([x, rate_diff_emb, content_id_rate_emb, user_id_rate_emb], dim=2)
...
def main(params: dict,
    ...
    column_config = {
        ...
        "user_id_rating": {"type": "numeric"},
        "content_rating": {"type": "numeric"}
    }
    ...
    if not load_pickle or is_debug:
        ...
        df["content_rating"] = df["content_rating"].replace(-1, 1397)  # content_id_mean
        df["user_id_rating"] = df["user_id_rating"].replace(-1, 1538)  # user_id_mean
    ...
    os.makedirs("../input/feature_engineering/model216_2", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model216_2/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model216_2/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model216_2/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model216_2/val.pickle", "rb") as f:
