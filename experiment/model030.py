...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128):
        ...
        self.multi_att_enc = nn.MultiheadAttention(embed_dim=embed_dim, num_heads=8, dropout=0.2)
    ...
    def forward(self, x, question_ids, parts):
        ...
        att_enc, _ = self.multi_att_enc(e, e, e, attn_mask=att_mask)
        att_enc = att_enc.permute(1, 0, 2)
        enc = self.ffn_enc(att_enc)
        enc = self.layer_normal(enc)

        x = x.permute(1, 0, 2)  # x: [bs, s_len, embed] => [s_len, bs, embed]
        enc = enc.permute(1, 0, 2)

        att_output, att_weight = self.multi_att_dec(enc, x, x, attn_mask=att_mask)
        att_output = self.layer_normal(att_output + enc)
