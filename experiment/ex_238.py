...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    df["timediff-elapsedtime"] = df["duration_previous_content_cap100k"] - df["elapsed_time_content_id_mean"]
    df["timediff-elapsedtime-mean"] = df["duration_previous_content_cap100k"] - df["elapsed_time_mean_content_id_vs_prior_elapsed_time"]

    df["usermean-studytime1"] = df.groupby("user_id")["elapsed_time_mean_content_id_vs_prior_elapsed_time"].cumsum() / df.groupby("user_id").cumcount()
    df["timediff-usermean"] = df["duration_previous_content_cap100k"] - df["usermean-studytime1"]
    df["usermean-studytime2"] = df.groupby("user_id")["timediff-elapsedtime-mean"].cumsum() / df.groupby("user_id").cumcount()
    df["timediff-usermean2"] = df["duration_previous_content_cap100k"] - df["usermean-studytime2"]
