...
from experiment import model302
model_dir = {
    ...
    "model302": "../output/model302/20210107150104"
}
...
class KurupicalModel:
    def __init__(self,
        ...
        self.models = {
            ...
            "model302": get_model(model_dir=model_dir["model302"], model_class=model302.SAKTModel),
        }

        # dataloaderは今回transformer, lstm共通
        model_dir = model_dir["model302"]
    ...
    def predict(self, df_test):
        ...
        dataset_val = model302.SAKTDataset(group, 13939, predict_mode=True, max_seq=self.params["max_seq"])
...
def run(model_dir,
    ...
    logger = get_logger()
    df_test_prev = None
    df_all = pd.read_feather("../input/riiid-test-answer-prediction/train.feather").head(1000000)
    df_val_row = pd.read_feather(
        "../input/riiid-test-answer-prediction/train_transformer_last2500k_only_row_id.feather"
    )

    df_all = pd.merge(df_all, df_val_row, how="inner")
    df_all = df_all.groupby("user_id").head(30)
    df_all["count"] = df_all.groupby("user_id")["user_id"].cumcount()

    print(len(df_all))
    ret = []
    i = 0
    for _, df_test in df_all.groupby(["count"]):
        ...
        # df_sample_prediction = df_test[df_test["content_type_id"] == 0][["row_id", "answered_correctly"]]
        # df_sample_prediction["pred"] = predicts["transformer"] * 0.5 + predicts["lstm"]* 0.5
        # ret.append(df_sample_prediction)
        i += 1
        if i >= 25:
            break
    ...
    print(roc_auc_score(df_ret["answered_correctly"].values, df_ret["pred"].values))
