...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    df["content_mean_elapsed_time"] = df.groupby("content_id")["elapsed_time"].transform("mean")
    df["timediff_elapsed"] = df["shiftdiff_timestamp_by_user_id"] - df["content_mean_elapsed_time"]

    df = df.drop(["elapsed_time", "content_mean_elapsed_time"], axis=1)
