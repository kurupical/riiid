...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2):
        ...
        self.lstm1 = nn.LSTM(input_size=embed_dim, hidden_size=embed_dim)
        self.lstm2 = nn.LSTM(input_size=embed_dim, hidden_size=embed_dim)
    ...
    def forward(self, item, device):
        ...
        out = torch.cat([att_dec, lstm_out], dim=2)
        x = self.layer_normal(out)
        x = self.ffn(x) + out
        x = self.dropout(x)
        x = self.pred(x)
...
def main(params: dict,
    ...
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model142/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model142/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model142/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model142/val.pickle", "rb") as f:
