...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        timediff_elapsedtime_ = self.samples[user_id]["timediff-elapsedtime_bin300_2"]
        ...
        if not self.is_test:
            ...
            timediff_elapsedtime_ = timediff_elapsedtime_[start:end]
            seq_len = len(q_)
        ...
        timediff_elapsedtime = np.zeros(self.max_seq, dtype=int)
        if seq_len >= self.max_seq:
            ...
            timediff_elapsedtime[:] = timediff_elapsedtime_[-self.max_seq:]
        else:
            q[-seq_len:] = q_
            part[-seq_len:] = part_
            qa[-seq_len:] = qa_
            ...
            timediff_elapsedtime[-seq_len:] = timediff_elapsedtime_
        target_id = q[1:]
        part = part[1:]
        ...
        timediff_elapsedtime = timediff_elapsedtime[1:]
        return {
            "x": x,
            "user_answer": ua,
            "target_id": target_id,
            ...
            "timediff_elapsedtime": timediff_elapsedtime
        }
...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        self.timediff_elapsedtime_embedding = nn.Embedding(302, 32)
    ...
    def forward(self, x, question_ids, parts, elapsed_time, duration_previous_content, prior_q, user_answer,
                rate_diff, container_id, prev_ans_idx, prev_ans_content_id, timediff_elapsedtime):
        device = x.device
        ...
        x = torch.cat([x, el_time_emb, dur_emb, e, p, prior_q_emb, user_answer_emb, container_emb,
                       prev_ans_idx_emb, prev_ans_content_id_emb, timediff_elapsedtime_emb], dim=2)
...
def train_epoch(model, train_iterator, val_iterator, optim, criterion, scheduler, epoch, device="cuda"):
    ...
    for item in tbar:
        ...
        timediff_elapsedtime = item["timediff_elapsedtime"].to(device).long()
...
def main(params: dict,
    ...
    column_config = {
        ...
        "timediff-elapsedtime_bin300_2": {"type": "category"}
    }
    ...
    if not load_pickle or is_debug:
        ...
        df["timediff-elapsedtime_bin300_2"] = [f(x) for x in df["timediff-elapsedtime"]]
        print(df["timediff-elapsedtime"].value_counts())
        print(df["timediff-elapsedtime_bin300_2"].value_counts())
    ...
    os.makedirs("../input/feature_engineering/model211_3", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model211_3/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model211_3/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model211_3/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model211_3/val.pickle", "rb") as f:
