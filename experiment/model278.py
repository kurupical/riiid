...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        self.layer_normal = nn.LayerNorm(embed_dim_all*2)

        self.ffn = FFN(embed_dim_all*2)
        self.dropout = nn.Dropout(dropout/2)
        self.pred = nn.Linear(embed_dim_all*2, 1)
    ...
    def forward(self, item, device):
        ...
        cat_emb = self.cat_embedding(x)
        enc = torch.cat([cat_emb, cont_emb], dim=2)

        enc_in = enc.permute(1, 0, 2)  # x: [bs, s_len, embed] => [s_len, bs, embed]

        att_dec = self.lstm_block(enc_in)
        att_dec = att_dec.permute(1, 0, 2)  # att_output: [s_len, bs, embed] => [bs, s_len, embed]

        att_dec = torch.cat([att_dec, enc], dim=2)
