...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2):
        ...
        self.continuous_embedding = ContEmbedding(input_dim=3, embed_dim=embed_dim, seq_len=max_seq)
    ...
    def forward(self, x, question_ids, parts, elapsed_time, duration_previous_content, prior_q, user_answer,
        ...
        x = torch.cat([x, e, p, prior_q_emb, user_answer_emb, container_emb], dim=2)

        cont = torch.cat([rate_diff.view(x.size(0), x.size(1), -1),
                          elapsed_time.view(x.size(0), x.size(1), -1),
                          duration_previous_content.view(x.size(0), x.size(1), -1)], dim=2)

        cont_emb = self.continuous_embedding(cont)
...
def main(params: dict,
    ...
    os.makedirs("../input/feature_engineering/model149", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model149/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model149/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model149/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model149/val.pickle", "rb") as f:
