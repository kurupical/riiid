...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        idx_dict = {
            ...
            "timedelta_log10": 12,
            "previous_answer_index_question_id_0": 13,
            "previous_answer_index_question_id_1": 14,
            "previous_answer_index_question_id_2": 15,
            "previous_answer_index_question_id_3": 16,
            "previous_answer_index_question_id_4": 17,
            "previous_answer_question_id_0": 18,
            "previous_answer_question_id_1": 19,
            "previous_answer_question_id_2": 20,
            "previous_answer_question_id_3": 21,
            "previous_answer_question_id_4": 22,
        }
        ...
        return {
            ...
            "previous_answer_index_question_id_0": get_data(key="previous_answer_index_question_id_0"),
            "previous_answer_index_question_id_1": get_data(key="previous_answer_index_question_id_1"),
            "previous_answer_index_question_id_2": get_data(key="previous_answer_index_question_id_2"),
            "previous_answer_index_question_id_3": get_data(key="previous_answer_index_question_id_3"),
            "previous_answer_index_question_id_4": get_data(key="previous_answer_index_question_id_4"),
            "previous_answer_question_id_0": get_data(key="previous_answer_question_id_0"),
            "previous_answer_question_id_1": get_data(key="previous_answer_question_id_1"),
            "previous_answer_question_id_2": get_data(key="previous_answer_question_id_2"),
            "previous_answer_question_id_3": get_data(key="previous_answer_question_id_3"),
            "previous_answer_question_id_4": get_data(key="previous_answer_question_id_4"),
        }
...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        embed_dim_small_cat = 16
        embed_dim_middle_cat = 32
        embed_dim_cat_all = embed_dim_small_cat*10 + embed_dim_middle_cat*9 + embed_dim
        ...
        self.prev_ans_idx_embeddings = [nn.Embedding(502, embed_dim_middle_cat).to("cuda") for _ in range(5)]
        self.prev_ans_content_id_embeddings = [nn.Embedding(4, embed_dim_small_cat).to("cuda") for _ in range(5)]
        ...
        self.all_embedding = nn.Sequential(
            nn.Linear(embed_dim_all, embed_dim_all),
            nn.LayerNorm(embed_dim_all)
        )
    ...
    def forward(self, item, device):
        ...
        prev_ans_idx_embs = []
        prior_content_id_ans_correctly_embs = []
        for i in range(5):
            ans_idx = item[f"previous_answer_index_question_id_{i}"].to(device).long()
            ans_corr = item[f"previous_answer_question_id_{i}"].to(device).long()
            prev_ans_idx_embs.append(self.prev_ans_idx_embeddings[i](ans_idx))
            prior_content_id_ans_correctly_embs.append(self.prev_ans_content_id_embeddings[i](ans_corr))
        ...
        x = torch.cat([x, el_time_emb, dur_emb, e, p, prior_q_emb, container_emb,
                       ...
                       prev_ans_idx_embs + prior_content_id_ans_correctly_embs, dim=2)
...
def train_epoch(model, train_iterator, val_iterator, optim, criterion, scheduler, epoch, output_dir, device="cuda"):
    model.train()
    ...
    df_oof.to_csv(f"{output_dir}/epoch{epoch}.csv")
...
def main(params: dict,
    ...
    column_config = {
        ...
        "previous_answer_index_question_id_0": {"type": "category"},
        "previous_answer_index_question_id_1": {"type": "category"},
        "previous_answer_index_question_id_2": {"type": "category"},
        "previous_answer_index_question_id_3": {"type": "category"},
        "previous_answer_index_question_id_4": {"type": "category"},
        "previous_answer_question_id": {"type": "category"},
        "previous_answer_question_id_0": {"type": "category"},
        "previous_answer_question_id_1": {"type": "category"},
        "previous_answer_question_id_2": {"type": "category"},
        "previous_answer_question_id_3": {"type": "category"},
        "previous_answer_question_id_4": {"type": "category"},
    ...
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model275_all/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model275_all/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model275_all/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model275_all/val.pickle", "rb") as f:
    ...
    for dropout in [0.2]:
        for num_warmup_stemps in [3000]:
            ...
            params = {"embed_dim": cat_emb,
                      ...
                      "num_warmup_steps": num_warmup_stemps,
                      "lr": lr,
                      "dropout": dropout}
            set_seed(0)
