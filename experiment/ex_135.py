...
def make_feature_factory_manager(split_num, model_id=None):
    ...
    for column in ["user_id", "content_id", "tags1", "tags2"]:
...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    window = 10
    for col in ["user_id_rating"]:
        df[f"{col}_mean{window}"] = df.groupby("user_id")[col].rolling(window=10).mean().values
        df[f"{col}_max{window}"] = df.groupby("user_id")[col].rolling(window=10).max().values
        df[f"{col}_min{window}"] = df.groupby("user_id")[col].rolling(window=10).min().values
        df[f"{col}_std{window}"] = df.groupby("user_id")[col].rolling(window=10).std().values

    df[f"diff_rating_user_id_{window}_mean_content_id"] = df[f"user_id_rating_mean{window}"] - df["content_rating"]
    df[f"diff_rating_user_id_{window}_max_content_id"] = df[f"user_id_rating_max{window}"] - df["content_rating"]
    df[f"diff_rating_user_id_{window}_min_content_id"] = df[f"user_id_rating_min{window}"] - df["content_rating"]
