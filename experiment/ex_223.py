...
def make_feature_factory_manager(split_num, model_id=None):
    ...
    for column in ["user_id", "content_id", ("user_id", "kmeans_tags20")]:
...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    for p in [1, 2, 3, 4, 5, 6, 7]:
        ...
        df_question[f"part_{p}"] = (df_question["part"] == p).astype("uint8")
    df_question["kmeans_tags20"] = KMeans(n_clusters=20).fit_predict(df_question[target_cols].values)

    df = pd.merge(df, df_question[["question_id", "kmeans_tags20"]], how="left")

    print("question clustering finish!")
