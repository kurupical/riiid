...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        diff_timedelta_groupby_user_id_part_ = self.samples[user_id]["diff_timedelta_groupby_user_id_part"]
        ...
        if not self.is_test:
            ...
            diff_timedelta_groupby_user_id_part_ = diff_timedelta_groupby_user_id_part_[start:end]
            seq_len = len(q_)
        ...
        diff_timedelta_groupby_user_id_part = np.zeros(self.max_seq, dtype=int)
        if seq_len >= self.max_seq:
            ...
            diff_timedelta_groupby_user_id_part[:] = diff_timedelta_groupby_user_id_part_[-self.max_seq:]
        else:
            q[-seq_len:] = q_
            part[-seq_len:] = part_
            qa[-seq_len:] = qa_
            ...
            diff_timedelta_groupby_user_id_part[-seq_len:] = diff_timedelta_groupby_user_id_part_
        ...
        diff_timedelta_groupby_user_id_part = diff_timedelta_groupby_user_id_part[1:]
        ...
        return {
            ...
            "diff_timedelta_groupby_user_id_part": diff_timedelta_groupby_user_id_part
        }
...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        self.diff_timedelta_groupby_user_id_part_embedding = nn.Embedding(302, 32)
    ...
    def forward(self, x, question_ids, parts, elapsed_time, duration_previous_content, prior_q, user_answer,
                rate_diff, container_id, prev_ans_idx, prev_ans_content_id, diff_timedelta_groupby_user_id_part):
        device = x.device
        ...
        diff_timedelta_groupby_user_id_part_emb = self.diff_timedelta_groupby_user_id_part_embedding(diff_timedelta_groupby_user_id_part)
        x = torch.cat([x, el_time_emb, dur_emb, e, p, prior_q_emb, user_answer_emb, container_emb,
                       prev_ans_idx_emb, prev_ans_content_id_emb,
                       diff_timedelta_groupby_user_id_part_emb], dim=2)
...
def train_epoch(model, train_iterator, val_iterator, optim, criterion, scheduler, epoch, device="cuda"):
    ...
    for item in tbar:
        ...
        diff_timedelta_groupby_user_id_part = item["diff_timedelta_groupby_user_id_part"].to(device).long()
        ...
        output = model(x, target_id, part, elapsed_time,
                       ...
                       diff_timedelta_groupby_user_id_part)
...
def main(params: dict,
    ...
    column_config = {
        ...
        "diff_timedelta_groupby_user_id_part": {"type": "category"}
    }
    ...
    if not load_pickle or is_debug:
        ...
        df["diff_timedelta_groupby_user_id_part"] = \
        ...
        print(df["diff_timedelta_groupby_user_id_part"].value_counts())
    ...
    os.makedirs("../input/feature_engineering/model215", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model215/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model215/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model215/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model215/val.pickle", "rb") as f:
