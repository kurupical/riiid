    ...
    def __getitem__(self, index):
        ...
        if not self.is_test:
            ...
            if random.random() < 0.1:
                seq_len -= int(seq_len * random.random())
                q_ = q_[-seq_len:]
                part_ = part_[-seq_len:]
                qa_ = qa_[-seq_len:]
                ua_ = ua_[-seq_len:]
                elapsed_time_ = elapsed_time_[-seq_len:]
                duration_previous_content_ = duration_previous_content_[-seq_len:]
                prior_q_ = prior_q_[-seq_len:]
                rate_diff_ = rate_diff_[-seq_len:]
                container_id_ = container_id_[-seq_len:]
                prev_ans_idx_ = prev_ans_idx_[-seq_len:]
...
class SAKTModel(nn.Module):
    ...
    def forward(self, x, question_ids, parts, elapsed_time, duration_previous_content, prior_q, user_answer,
        ...
        x = torch.cat([x, el_time_emb, dur_emb, e, p, prior_q_emb, user_answer_emb, container_emb, prev_ans_idx_emb], dim=2)
...
def main(params: dict,
    ...
    os.makedirs("../input/feature_engineering/model151", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model151/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model151/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model151/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model151/val.pickle", "rb") as f:
