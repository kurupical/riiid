...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model=embed_dim_cat_all, nhead=1)
        self.cat_embedding = nn.TransformerEncoder(encoder_layer=encoder_layer, num_layers=1)
    ...
    def forward(self, item, device):
        ...
        cont_emb = cont_emb.permute(1, 0, 2)
        x = self.cat_embedding(x, mask=att_mask)
