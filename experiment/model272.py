...
class LSTMBlock(nn.Module):
    def __init__(self, input_dim, embed_dim, dropout, nhead=8):
        super(LSTMBlock, self).__init__()
        if input_dim % nhead != 0: raise ValueError(f"input_dim{input_dim} % nhead{nhead} != 0. ")
        self.lstms = [nn.LSTM(input_size=input_dim//nhead, hidden_size=embed_dim//nhead).to("cuda") for _ in range(nhead)]
        self.layer_norm = nn.LayerNorm(embed_dim)
        self.dropout = nn.Dropout(dropout)
        self.nhead = nhead

    def forward(self, x):
        """x: shape = (s_len, bs, embed) """
        def f(model, x, idx):
            idx_start = idx * x.shape[2] // self.nhead
            idx_end = (idx+1) * x.shape[2] // self.nhead

            return model(x[:, :, idx_start:idx_end])[0]

        lstm = torch.cat([f(model, x, idx) for idx, model in enumerate(self.lstms)], dim=2)
