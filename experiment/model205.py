...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        tags1_ = self.samples[user_id]["tags1"]
        tags2_ = self.samples[user_id]["tags2"]
        tags3_ = self.samples[user_id]["tags3"]
        tags4_ = self.samples[user_id]["tags4"]
        ...
        if not self.is_test:
            ...
            tags1_ = tags1_[start:end]
            tags2_ = tags2_[start:end]
            tags3_ = tags3_[start:end]
            tags4_ = tags4_[start:end]
            seq_len = len(q_)
        ...
        tags1 = np.zeros(self.max_seq, dtype=int)
        tags2 = np.zeros(self.max_seq, dtype=int)
        tags3 = np.zeros(self.max_seq, dtype=int)
        tags4 = np.zeros(self.max_seq, dtype=int)
        if seq_len >= self.max_seq:
            ...
            tags1[:] = tags1_[-self.max_seq:]
            tags2[:] = tags2_[-self.max_seq:]
            tags3[:] = tags3_[-self.max_seq:]
            tags4[:] = tags4_[-self.max_seq:]
        else:
            q[-seq_len:] = q_
            part[-seq_len:] = part_
            qa[-seq_len:] = qa_
            ...
            tags1[-seq_len:] = tags1_
            tags2[-seq_len:] = tags2_
            tags3[-seq_len:] = tags3_
            tags4[-seq_len:] = tags4_
...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        self.tags_embedding = nn.Embedding(512, 32)
    ...
    def forward(self, item, device):
        ...
        tags1 = item["tags1"].to(device).long()
        tags2 = item["tags1"].to(device).long()
        tags3 = item["tags1"].to(device).long()
        tags4 = item["tags1"].to(device).long()
        ...
        tags_emb = \
            self.tags_embedding(tags1) + \
            self.tags_embedding(tags2) + \
            self.tags_embedding(tags3) + \
            self.tags_embedding(tags4)

        x = torch.cat([x, el_time_emb, dur_emb, e, p, prior_q_emb, container_emb,
                       content_id_delta_emb, last_content_id_acc_emb, tags_emb], dim=2)
...
def main(params: dict,
    ...
    tags_dict = df_question.groupby("question_id")["tags"].sum().to_dict()
    df["tags"] = np.nan
    df.loc[df["content_type_id"] == 0, "tags"] = df.loc[df["content_type_id"] == 0, "content_id"].map(tags_dict)

    tag = df["tags"].str.split(" ", n=3, expand=True)
    tag.columns = [f"tags{i}" for i in range(1, len(tag.columns) + 1)]

    for col in ["tags1", "tags2", "tags3", "tags4"]:
        if col in tag.columns:
            df[col] = pd.to_numeric(tag[col], errors='coerce').fillna(-1).astype("uint16") + 2
        else:
            df[col] = -1
            df[col].astype("int16")

    print(df[["tags1", "tags2", "tags3", "tags4"]])
    ...
    os.makedirs("../input/feature_engineering/model205", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model205/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model205/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model205/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model205/val.pickle", "rb") as f:
