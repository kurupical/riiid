...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        self.layer_normal_cont = nn.LayerNorm(max_seq-1)
        self.layer_normal = nn.LayerNorm(embed_dim_all + 1)

        self.ffn = FFN(embed_dim_all + 1)
        self.dropout = nn.Dropout(dropout/2)
        self.pred = nn.Linear(embed_dim_all + 1, 1)
    ...
    def forward(self, item, device):
        ...
        rate_diff = self.layer_normal_cont(rate_diff)
        ...
        att_dec = torch.cat([att_dec, rate_diff.view(rate_diff.shape[0], rate_diff.shape[1], -1)], dim=2)
