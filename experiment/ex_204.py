...
def make_feature_factory_manager(split_num, model_id=None):
    ...
    feature_factory_dict["user_id"]["PastNTimestampEncoder"] = PastNFeatureEncoder(column="timestamp",
                                                                                   ...
                                                                                   agg_funcs=["vslast"],
                                                                                   remove_now=False)
    feature_factory_dict["user_id"]["PastNAnsweredCorrectly"] = PastNFeatureEncoder(column="answered_correctly",
                                                                                    past_ns=[50, 100],
                                                                                    agg_funcs=["mean"],
                                                                                    remove_now=True)
...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    df.tail(1_000_000).to_csv("exp028.csv", index=False)

    df = df.drop(["user_answer", "tags", "type_of", "bundle_id", "previous_5_ans", "tag", "content_type_id"], axis=1)
