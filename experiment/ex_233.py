...
def make_feature_factory_manager(split_num, model_id=None):
    ...
    feature_factory_dict["user_id"]["ElapsedTimeMeanByContentIdEncoder"] = ElapsedTimeMeanByContentIdEncoder(is_partial_fit=True)
    ...
    for column in ["user_id", "content_id", "part", ("user_id", "part")]:
        ...
        if type(column) == str:
            ...
            feature_factory_dict[column][f"MeanAggregatorStudyTimeby{column}"] = MeanAggregator(column=column,
                                                                                                ...
                                                                                                remove_now=False)
            feature_factory_dict[column][f"MeanAggregatorElapsedTimeDiffby{column}"] = MeanAggregator(column=column,
                                                                                                      agg_column="elapsed_time_mean_content_id_vs_prior_elapsed_time",
                                                                                                      remove_now=False)
        else:
            ...
            feature_factory_dict[column][f"MeanAggregatorStudyTimeby{column}"] = MeanAggregator(column=list(column),
                                                                                                agg_column="study_time",
                                                                                                remove_now=False)
            feature_factory_dict[column][f"MeanAggregatorElapsedTimeDiffby{column}"] = MeanAggregator(column=list(column),
                                                                                                      agg_column="elapsed_time_mean_content_id_vs_prior_elapsed_time",
                                                                                                      remove_now=False)
...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    df["predict_elapsed_time"] = df["elapsed_time_content_id_mean"] + df["mean_elapsed_time_mean_content_id_vs_prior_elapsed_time_by_['user_id', 'part']"]
