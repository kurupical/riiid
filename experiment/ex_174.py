...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    for part in [1, 2, 3, 4, 5, 6, 7]:
        df[f"part{part}"] = (df["part"] == part).replace(0, np.nan) * df["answered_correctly"].values
        df["shift1"] = df.groupby("user_id")[f"part{part}"].shift(1)
        df["notnull"] = df["shift1"].notnull()

        df[f"{part}_mean"] = df.groupby("user_id")["shift1"].cumsum() / df.groupby("user_id")["notnull"].cumsum()
        df = df.drop([f"part{part}", "shift1", "notnull"], axis=1)
