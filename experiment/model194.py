...
from model.optim import AdaBelief, RAdam
...
class ContEmbedding(nn.Module):
    def __init__(self, input_dim, embed_dim, seq_len):
        ...
        self.bn = nn.BatchNorm1d(seq_len-1)
        self.gru = nn.Linear(input_dim, embed_dim)
...
def train_epoch(model, train_iterator, val_iterator, optim, criterion, epoch, device="cuda"):
    model.train()
...
def main(params: dict,
    ...
    os.makedirs("../input/feature_engineering/model158_all", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model158_all/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model158_all/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model158_all/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model158_all/val.pickle", "rb") as f:
            dataset_val = pickle.load(f)
        print("loaded!")
    dataloader_train = DataLoader(dataset_train, batch_size=params["batch_size"], shuffle=True, num_workers=4)
    dataloader_val = DataLoader(dataset_val, batch_size=params["batch_size"], shuffle=False, num_workers=4)
    ...
    for epoch in range(epochs):
        loss, acc, auc, auc_val = train_epoch(model, dataloader_train, dataloader_val, optimizer, criterion,
                                              epoch, device)
