...
class SAKTModel(nn.Module):
    ...
    def forward(self, x, question_ids, parts, tsdiff, elptime):
        ...
        pos_x1 = self.pos_embedding1(pos_id)
        x = x + pos_x1 + p

        e = self.e_embedding(question_ids)
        pos_x2 = self.pos_embedding2(pos_id)
...
def main(params: dict):
    ...
    df = pd.read_pickle("../input/riiid-test-answer-prediction/train_merged.pickle").sort_values(["user_id", "timestamp"]).reset_index(drop=True)
    if is_debug:
        df = df.head(10000)
