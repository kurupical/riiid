...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        # common
        # --------------
        self.positional_encoding = nn.Embedding(max_seq, embed_dim)

        # --------------
        # previous layer
        # --------------
        self.answered_correctly_embedding = nn.Embedding(4, embed_dim)
        ...
        previous_layer = nn.TransformerEncoderLayer(d_model=embed_dim, nhead=num_heads, dropout=dropout)
        self.transformer_previous = nn.TransformerEncoder(encoder_layer=previous_layer, num_layers=2)
        ...
        self.container_embedding = nn.Embedding(302, embed_dim)
        self.prev_ans_idx_embedding = nn.Embedding(302, embed_dim)
        self.prev_ans_content_id_embedding = nn.Embedding(4, embed_dim)
        self.decoder_embedding = nn.Sequential(
            nn.LayerNorm(embed_dim),
            nn.Linear(embed_dim, embed_dim),
            nn.LayerNorm(embed_dim)
        )
        now_layer = nn.TransformerEncoderLayer(d_model=embed_dim, nhead=num_heads, dropout=dropout)
        self.transformer_now = nn.TransformerEncoder(encoder_layer=now_layer, num_layers=2)

        # ----------------
        # transformer layer
        # ----------------
        concat_layer = nn.TransformerEncoderLayer(d_model=embed_dim*2, nhead=num_heads*2, dropout=dropout)
        self.transformer_concat = nn.TransformerEncoder(encoder_layer=concat_layer, num_layers=1)
    ...
    def forward(self, item, device):
        ...
        previous = (
            self.answered_correctly_embedding(x) +
            self.user_answer_embedding(user_answer) +
            self.elapsed_time_embedding(elapsed_time)
        ).permute(1, 0, 2)
        previous = self.transformer_previous(previous,
                                             mask=att_mask)

        now = (
            self.part_embedding(parts) +
            self.content_id_embedding(question_ids) +
            self.duration_previous_content_embedding(duration_previous_content) +
            self.content_id_embedding(container_id) +
            self.prev_ans_idx_embedding(prev_ans_idx) +
            self.prev_ans_content_id_embedding(prior_content_id_ans_correctly)
        ).permute(1, 0, 2)
        now = self.transformer_now(now,
                                   mask=att_mask)

        x = torch.cat([previous, now], dim=2)
        x = self.transformer_concat(x, mask=att_mask)
        x, _ = self.gru(x)
        x = x.permute(1, 0, 2)  # att_output: [s_len, bs, embed] => [bs, s_len, embed]

        out = self.layer_normal(x)
        out = self.ffn(out) + x
...
def main(params: dict,
    ...
    os.makedirs("../input/feature_engineering/model220", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model220/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model220/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model220/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model220/val.pickle", "rb") as f:
