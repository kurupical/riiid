...
def make_feature_factory_manager(split_num):
    ...
    feature_factory_dict["user_id"]["PreviousAnswer2"] = PreviousAnswer2(groupby="user_id", column="content_id", is_debug=is_debug, repredict=True)
...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    for lr in [0.01, 0.03]:
        for depth in [11, 12, 13, 14, 15, 16]:
            params = {
                'n_estimators': 12000,
                'learning_rate': lr,
                'eval_metric': 'AUC',
