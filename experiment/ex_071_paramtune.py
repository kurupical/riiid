...
def make_feature_factory_manager(split_num, model_id=None):
    ...
    feature_factory_dict["user_id"]["CategoryLevelEncoderPart"] = CategoryLevelEncoder(groupby_column="user_id",
                                                                                       agg_column="part",
                                                                                       categories=[2, 5])
    feature_factory_dict["user_id"]["FirstColumnEncoderContentId"] = FirstColumnEncoder(column="content_id",
                                                                                        astype="int16",
                                                                                        is_partial_fit=True)
    feature_factory_dict["user_id"]["FirstColumnEncoderPart"] = FirstColumnEncoder(column="part",
                                                                                   astype="int8",
                                                                                   is_partial_fit=True)
    for column in ["user_id", "user_count_bin", "first_column_content_id", "first_column_part", ("user_id", "part")]:
        if column not in feature_factory_dict:
            if type(column) == str:
                feature_factory_dict[column] = {}
                feature_factory_dict[column]["CountEncoder"] = CountEncoder(column=column,
                                                                            is_partial_fit=is_partial_fit)
            else:
                feature_factory_dict[column] = {}
                feature_factory_dict[column]["CountEncoder"] = CountEncoder(column=list(column),
                                                                            is_partial_fit=is_partial_fit)
        if type(column) == str:
    ...
    for column in ["content_id", "part", "tags1", "tags2",
...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    for _ in range(10000):
        params = {
            'objective': 'binary',
            'num_leaves': random.choice([32, 64, 96, 128]),
            'max_depth': random.choice([7, 8, 9, 10, 11]),
            'learning_rate': 0.3,
