...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        encoder_layer_cat = nn.TransformerEncoderLayer(d_model=embed_dim_cat_all, nhead=1, dropout=dropout)
        self.cat_embedding = nn.TransformerEncoder(encoder_layer=encoder_layer_cat, num_layers=1)
...
def main(params: dict,
    ...
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model256/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model256/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model256/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model256/val.pickle", "rb") as f:
