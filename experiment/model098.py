...
class SAKTModel(nn.Module):
    ...
    def forward(self, x, question_ids, parts, elapsed_time, duration_previous_content, prior_q, user_answer,
        ...
        cat_emb = torch.cat([el_time_emb, dur_emb, e, p, prior_q_emb, user_answer_emb], dim=2)
        ...
        cat_emb = self.cat_embedding(cat_emb)
        enc = torch.cat([cat_emb, cont_emb], dim=2)

        enc = enc.permute(1, 0, 2)  # x: [bs, s_len, embed] => [s_len, bs, embed]

        att_enc = self.transformer_enc(enc,
                                       mask=att_mask)
        target = self.embedding(x)
        target = target.permute(1, 0, 2)

        att_dec = self.transformer_dec(tgt=target,
                                       memory=att_enc)
...
def main(params: dict,
    ...
    os.makedirs("../input/feature_engineering/model097", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model097/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model097/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model097/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model097/val.pickle", "rb") as f:
