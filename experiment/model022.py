...
class SAKTDataset(Dataset):
    def __init__(self, group, n_skill, n_part=8, max_seq=100, is_test=False):
        ...
        for user_id in group.index:
            q, part, qa, tsdiff, is_val = group[user_id]
            if not is_test:
    ...
    def __getitem__(self, index):
        ...
        q_, part_,tsdiff_, qa_,  _ = self.samples[user_id]
        ...
        return x, target_id, part, tsdiff, label
...
class SAKTModel(nn.Module):
    ...
    def forward(self, x, question_ids, parts, tsdiff):
        device = x.device
        x = self.embedding(x)
...
def train_epoch(model, train_iterator, val_iterator, optim, criterion, device="cuda"):
    ...
    for item in tbar:
        ...
        optim.zero_grad()
        output, atten_weight = model(x, target_id, part, tsdiff)
...
def main(params: dict):
    ...
    group = df[df["is_val"] == 0][['user_id', 'content_id', 'answered_correctly', "group", "part", 'is_val', "timestamp_diff"]].groupby(['user_id', "group"]).apply(lambda r: (
                r['content_id'].values,
                r['part'].values,
    ...
    group = df[['user_id', 'content_id', 'answered_correctly', "part", 'is_val', "timestamp_diff"]].groupby(['user_id']).apply(lambda r: (
                r['content_id'].values,
                r['part'].values,
                r["timestamp_diff"].values,
                r['answered_correctly'].values,
