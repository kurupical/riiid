...
from experiment.model014 import SAKTModel, SAKTDataset

warnings.filterwarnings("ignore")

model_dir = "../output/model016/20201205091106"
device = "cuda"
...
def run(debug,
    ...
    for (df_test, df_sample_prediction) in iter_test:
        ...
        n_skill = len(feature_factory_manager_for_transformer.embbed_dict[("content_id", "content_type_id")])

        df = df_test
        group = feature_factory_manager_for_transformer.partial_predict(df_test[df_test["content_type_id"] == 0])
        logger.info(f"------ predict ------")

        dataset_val = SAKTDataset(group, n_skill, predict_mode=True, max_seq=params["max_seq"])
