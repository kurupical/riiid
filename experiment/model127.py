...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
                 cat_emb1=None, cat_emb2=None):
        super(SAKTModel, self).__init__()
        self.n_skill = n_skill
        self.embed_dim = embed_dim
        embed_dim = cat_emb1*4 + cat_emb2*2 + 256

        self.embedding = nn.Embedding(4, cat_emb1)
        self.user_answer_embedding = nn.Embedding(6, cat_emb1)
        self.prior_question_had_explanation_embedding = nn.Embedding(4, cat_emb1)
        self.e_embedding = nn.Embedding(n_skill + 1, 256)
        self.part_embedding = nn.Embedding(8, cat_emb1)
        self.elapsed_time_embedding = nn.Embedding(302, cat_emb2)
        self.duration_previous_content_embedding = nn.Embedding(302, cat_emb2)
...
def main(params: dict,
    ...
    model = SAKTModel(n_skill, embed_dim=params["embed_dim"], max_seq=params["max_seq"], dropout=dropout,
                      cat_emb1=params["cat_emb1"], cat_emb2=params["cat_emb2"])
...
if __name__ == "__main__":
    ...
    for c in [[8, 8], [16, 16], [64, 64], [128, 128],
