...
class SAKTDataset(Dataset):
    def __init__(self, df, n_skill, n_part=8, max_seq=100, is_test=False, predict_mode=False):
        ...
        for user_id, w_df in df.groupby("user_id"):
            self.samples[user_id] = w_df
            if self.is_test:
                for i, is_val in enumerate(w_df["is_val"].values):
                    if is_val and i > 0:
                        self.user_ids.append([user_id, i])
            else:
    ...
    def __getitem__(self, index):
        ...
        end_idx = self.user_ids[index][1]
        q_ = self.samples[user_id]["content_id"].values
        ua_ = self.samples[user_id]["user_answer"].values
        part_ = self.samples[user_id]["part"].values
        elapsed_time_ = self.samples[user_id]["prior_question_elapsed_time_bin300"].values
        duration_previous_content_ = self.samples[user_id]["duration_previous_content_bin300"].values
        qa_ = self.samples[user_id]["answered_correctly"].values

        start = np.max([0, end_idx - self.max_seq])
        q_ = q_[start:end_idx]
        part_ = part_[start:end_idx]
        qa_ = qa_[start:end_idx]
        ua_ = ua_[start:end_idx]
        elapsed_time_ = elapsed_time_[start:end_idx]
        duration_previous_content_ = duration_previous_content_[start:end_idx]
        seq_len = len(q_)
...
class SAKTModel(nn.Module):
    ...
    def forward(self, x, question_ids, parts, elapsed_time, duration_previous_content):
        ...
        att_enc = self.transformer_enc(e)
...
def main(params: dict,
    ...
    if not load_pickle or is_debug:
        ...
        w_df = df[df["is_val"] == 0]
        w_df = w_df[w_df["content_type_id"] == 0]
        ...
        dataset_train = SAKTDataset(w_df,
                                    n_skill=n_skill,
    ...
    if not load_pickle or is_debug:
        dataset_val = SAKTDataset(df[df["content_type_id"] == 0],
                                  is_test=True,
                                  n_skill=n_skill,
                                  max_seq=params["max_seq"])

    os.makedirs("../input/feature_engineering/model062", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model062/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model062/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model062/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model062/val.pickle", "rb") as f:
