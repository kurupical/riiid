...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        # common
        self.content_id_embedding = nn.Embedding(n_skill+1, embed_dim)

        # encoder
        self.timedelta_embedding = nn.Embedding(302, embed_dim)
        self.container_id_embedding = nn.Embedding(302, embed_dim)
        self.part_embedding = nn.Embedding(8, embed_dim)
        self.rate_diff_embedding = ContEmbedding(input_dim=1, embed_dim=embed_dim, seq_len=max_seq)
        ...
        # decoder
        self.user_answer_embedding = nn.Embedding(6, embed_dim)
        self.prior_q_embedding = nn.Embedding(4, embed_dim)
        self.elapsed_time_embedding = nn.Embedding(302, embed_dim)
        self.label_decoder = nn.Embedding(4, embed_dim)
    ...
    def forward(self, item, device):
        ...
        cont_dec_emb = self.rate_diff_embedding(rate_diff.view(rate_diff.shape[0], rate_diff.shape[1], -1))

        dec = cat_dec_emb + cont_dec_emb
