...
def make_feature_factory_manager(split_num, model_id=None):
    ...
    feature_factory_dict[("user_id", "part")]["UserContentRateEncoder"] = UserContentRateEncoder(column=["user_id", "part"],
                                                                                                 rate_func="elo")
    feature_factory_dict[("user_id", "part")]["CountEncoder"] = CountEncoder(column=["user_id", "part"])

    feature_factory_dict[("user_id", "part")]["DurationPreviousContent"] = DurationPreviousContent(groupby=["user_id", "part"],
                                                                                                   is_partial_fit=True)
    ...
    feature_factory_dict["post"] = {
        ...
        "PostProcessSumEncoder": PostProcessSumEncoder()
    }
...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    df = df.drop(["user_answer", "tags", "type_of", "bundle_id", "previous_5_ans",
                  "tag", "content_type_id"], axis=1)
    df_train = df.iloc[:len(train_idx)]
    df_val = df.iloc[len(train_idx):]
    print(df_train)

    df2 = pd.read_pickle(fname).sort_values(["user_id", "timestamp"]).reset_index(drop=True).head(3000)
    ...
    df2_val = []
    # for _, w_df in df2.iloc[val_idx].groupby(["user_id", "task_container_id"], sort=False):
    for i in tqdm.tqdm(range(len(df2.iloc[val_idx]))):
    ...
    df_val.to_csv(f"{output_dir}/all.csv", index=False)
    df2_val.to_csv(f"{output_dir}/partial.csv", index=False)
