...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {
            ...
            "user_answer": ua,
            "target_id": target_id,
            "past_content_id": past_content_id,
            "part": part,
...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        embed_dim_cat_all = 32*8 + embed_dim*2
        embed_dim_all = embed_dim_cat_all//2 + cont_emb

        self.embedding = nn.Embedding(4, self.embed_dim_cat)
        ...
        self.cat_embedding = nn.Sequential(
            nn.Linear(embed_dim_cat_all, embed_dim_cat_all//2),
            nn.LayerNorm(embed_dim_cat_all//2)
        )
    ...
    def forward(self, x, question_ids, parts, elapsed_time, duration_previous_content, prior_q, user_answer,
                rate_diff, container_id, prev_ans_idx, prev_ans_content_id, past_content_id):
        device = x.device
        ...
        past_content_id_emb = self.e_embedding(past_content_id)

        past_emb = x + past_content_id_emb
        x = torch.cat([el_time_emb, dur_emb, e, p, prior_q_emb, user_answer_emb, container_emb,
                       prev_ans_idx_emb, prev_ans_content_id_emb, past_emb], dim=2)
...
def train_epoch(model, train_iterator, val_iterator, optim, criterion, scheduler, epoch, device="cuda"):
    ...
    for item in tbar:
        ...
        past_content_id = item["past_content_id"].to(device).long()
...
def main(params: dict,
    ...
    os.makedirs("../input/feature_engineering/model162", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model162/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model162/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model162/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model162/val.pickle", "rb") as f:
