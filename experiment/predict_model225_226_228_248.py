...
from experiment import model228  # gru*2 CV: 0.8092
from experiment import model248  # transformer_lit CV: 0.8094

model_dir = {
    "transformer": "../output/model225/20201229000607",
    "lstm": "../output/model226/20201229001216",
    "gru": "../output/model228/20201229105540",
    "transformer_lit": "../output/model248/202012311235453"
}
...
class KurupicalModel:
    def __init__(self,
        ...
        self.model_gru = get_model(model_dir=model_dir["gru"], model_class=model228.SAKTModel)
        self.model_transformer_lit = get_model(model_dir=model_dir["transformer_lit"], model_class=model248.SAKTModel)
    ...
    def predict(self, df_test):
        ...
        with torch.no_grad():
            ...
            for item in dataloader_val:
                ...
                # gru
                output = self.model_gru(item, device)
                predicts_gru.extend(torch.nn.Sigmoid()(output[:, -1]).view(-1).data.cpu().numpy().tolist())

                # transformer_lit
                output = self.model_transformer_lit(item, device)
                predicts_transformer_lit.extend(torch.nn.Sigmoid()(output[:, -1]).view(-1).data.cpu().numpy().tolist())

        ret_dict = {
            "transformer": np.array(predicts_transformer),
            "lstm": np.array(predicts_lstm),
            "gru": np.array(predicts_gru),
            "transformer_lit": np.array(predicts_transformer_lit)
        }
