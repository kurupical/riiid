...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label = qa[1:].copy() - 3
        label = label[-50:] # 最後の50件だけがloss計算対象
...
def main(params: dict,
    ...
    if not load_pickle or is_debug:
        ...
        group.update(w_df.groupby(["user_id", "group"]).agg(agg_dict).T.to_dict())

        # 50件ずらす
        w_df["group"] = (w_df.groupby("user_id")["user_id"].transform("count") + 50 - w_df.groupby("user_id").cumcount()) // params["max_seq"]
        w_df["group"] += 10000 # キー被らないように
        group.update(w_df.groupby(["user_id", "group"]).agg(agg_dict).T.to_dict())
    ...
    os.makedirs("../input/feature_engineering/model207", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model207/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model207/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model207/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model207/val.pickle", "rb") as f:
...
if __name__ == "__main__":
    ...
    for lr in [0.5e-3, 0.75e-3]:
        for num_warmup_steps in [1000, 2000]:
            cont_emb = 16
            cat_emb = 256
            dropout = 0.5
            if is_debug:
