...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    df["w_answer_ratio"] = \
        df.groupby(["content_id", "user_answer"])["content_id"].transform("count") / df.groupby("content_id")["content_id"].transform("count")
    ans = df.groupby(["content_id", "w_answer_ratio"]).size().sort_values(ascending=False)
    print(ans)
    ans = ans.reset_index().groupby("content_id")["w_answer_ratio"]
    df["ans_1_ratio"] = df["content_id"].map(ans.nth(0).to_dict())
    df["ans_2_ratio"] = df["content_id"].map(ans.nth(1).to_dict())
    df["ans_3_ratio"] = df["content_id"].map(ans.nth(2).to_dict())
    df["ans_4_ratio"] = df["content_id"].map(ans.nth(3).to_dict())
    print(df[["ans_1_ratio", "ans_2_ratio", "ans_3_ratio", "ans_4_ratio"]])
    df = df.drop("w_answer_ratio", axis=1)
