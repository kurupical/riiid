...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {
            ...
            "rating_diff_bin700": get_data(key="rating_diff_bin700"),
...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        embed_dim_cat_all = embed_dim_small_cat*5 + embed_dim_middle_cat*6 + embed_dim
        embed_dim_all = embed_dim_cat_all
        ...
        self.rate_diff_embedding = nn.Embedding(702, embed_dim_middle_cat)
    ...
    def forward(self, item, device):
        ...
        rate_diff = item["rating_diff_bin700"].to(device).long()
        ...
        rate_diff_emb = self.rate_diff_embedding(rate_diff)
        ...
        x = torch.cat([x, el_time_emb, dur_emb, e, p, prior_q_emb, container_emb,
                       ...
                       timediff_elapsedtime_emb, rate_diff_emb], dim=2)
...
def main(params: dict,
    ...
    column_config = {
        ...
        "rating_diff_bin700": {"type": "category"},
    ...
    if not load_pickle or is_debug:
        ...
        df["rating_diff_bin700"] = [g(x) for x in df["rating_diff_content_user_id"].values]
        df = df[["user_id", "content_id", "content_type_id", "part", "user_answer", "answered_correctly",
                 ...
                 "timediff-elapsedtime_bin500", "rating_diff_bin700"]]
        print(df.head(10))
    ...
    os.makedirs("../input/feature_engineering/model260", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model260/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model260/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model260/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model260/val.pickle", "rb") as f:
