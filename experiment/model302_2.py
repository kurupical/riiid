...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        self.prev_ans_idx_embeddings = [nn.Embedding(502, embed_dim_small_cat) for _ in range(5)]
        self.prev_ans_content_id_embeddings = [nn.Embedding(4, embed_dim_small_cat) for _ in range(5)]
    ...
    def forward(self, item, device):
        ...
        x = torch.cat([x, el_time_emb, dur_emb, e, p, prior_q_emb, container_emb,
                       prev_ans_idx_embs, prior_content_id_ans_correctly_embs,
...
def main(params: dict,
    ...
    column_config = {
        ...
        "previous_answer_index_question_id_5": {"type": "category"},
    ...
    os.makedirs("../input/feature_engineering/model302_all", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model302_all/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model302_all/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model302_all/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model302_all/val.pickle", "rb") as f:
