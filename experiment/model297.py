...
class SEModule(nn.Module):
    ...
    def forward(self, x):
        x = x.permute(0, 2, 1)  # (batch_size, sequence_len, embedding) -> (batch_size, embedding, sequence_len)
        s = self.avg_pool(x)  #
        s = self.conv(s)
        x *= torch.sigmoid(s)
        x = x.permute(0, 2, 1)
...
class LSTMBlockWithSE(nn.Module):
    def __init__(self, input_dim, embed_dim, dropout):
        super(LSTMBlockWithSE, self).__init__()
        self.lstm = nn.LSTM(input_size=input_dim, hidden_size=embed_dim)
        self.se_block = SEModule(in_channels=embed_dim)
        self.layer_norm = nn.LayerNorm(embed_dim)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x):
        lstm, _ = self.lstm(x)
        lstm = self.layer_norm(lstm)
        se = self.se_block(lstm)
        out = self.dropout(se)
...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        embed_dim_cat_all = embed_dim_small_cat*6 + embed_dim_middle_cat*5 + embed_dim
        ...
        self.lstm_block = nn.Sequential(
            LSTMBlockWithSE(input_dim=embed_dim_all, embed_dim=embed_dim_all//2, dropout=dropout),
            LSTMBlockWithSE(input_dim=embed_dim_all*6//4, embed_dim=embed_dim_all*3//4, dropout=dropout)
        )
        ...
        embed_dim_after_lstm = embed_dim_all*9//4

        self.layer_normal = nn.LayerNorm(embed_dim_after_lstm)

        self.ffn = FFN(embed_dim_after_lstm)
        self.dropout = nn.Dropout(dropout/2)
        self.pred = nn.Linear(embed_dim_after_lstm, 1)
...
def main(params: dict,
    ...
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model270/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model270/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model270/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model270/val.pickle", "rb") as f:
    ...
    optimizer_grouped_parameters = [
        {'params': [p for n, p in param_optimizer if not any(nd in n for nd in no_decay)], 'weight_decay': 0.2},
...
if __name__ == "__main__":
    ...
    for dropout in [0.2, 0.5]:
        for num_warmup_stemps in [3000, 10000, 20000]:
