...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128):
        ...
        self.embedding = nn.Embedding(5 * n_skill + 1, embed_dim)
        self.pos_embedding = nn.Embedding(max_seq - 1, embed_dim)
        self.e_embedding = nn.Embedding(n_skill + 1, embed_dim)
        self.part_embedding = nn.Embedding(8, embed_dim)

        self.multi_att = nn.MultiheadAttention(embed_dim=embed_dim, num_heads=8, dropout=0.2)
        self.multi_att_self = nn.MultiheadAttention(embed_dim=embed_dim, num_heads=8, dropout=0.2)

        self.dropout = nn.Dropout(0.2)
        self.layer_normal = nn.LayerNorm(embed_dim)

        self.ffn_dec_self = FFN(embed_dim)
    ...
    def forward(self, x, question_ids, parts):
        ...
        att_output_self, att_weight = self.multi_att_self(x, x, x, attn_mask=att_mask)
        att_output_self = self.ffn_dec_self(att_output_self.permute(1, 0, 2))
        att_output_self = self.layer_normal(att_output_self + x.permute(1, 0, 2)).permute(1, 0, 2)

        att_output, att_weight = self.multi_att(e, e, att_output_self, attn_mask=att_mask)
        att_output = self.layer_normal(att_output + x)
        att_output = att_output.permute(1, 0, 2)  # att_output: [s_len, bs, embed] => [bs, s_len, embed]

        x = self.ffn1(att_output)
