...
for fname in glob.glob("../input/riiid-test-answer-prediction/split10/*"):
    ...
    correct_dict = df[df["answered_correctly"] == 1].groupby("content_id")["duration_previous_content_cap100k"].mean().to_dict()
    incorrect_dict = df[df["answered_correctly"] == 0].groupby("content_id")["duration_previous_content_cap100k"].mean().to_dict()

    df["incorrect_duration_previous_content_cap100k_diff"] = df["duration_previous_content_cap100k"] - df["content_id"].map(incorrect_dict)
    df["correct_duration_previous_content_cap100k_diff"] = df["duration_previous_content_cap100k"] - df["content_id"].map(correct_dict)
    ...
    df = df.drop(["user_answer", "tags", "type_of", "bundle_id", "previous_5_ans",
                  "tag", "content_type_id", "content_past3"],
