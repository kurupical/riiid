...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        timediff_elapsedtime_ = self.samples[user_id]["timediff-elapsedtime_bin300"]
...
def main(params: dict,
    ...
    column_config = {
        ...
        "timediff-elapsedtime_bin300": {"type": "category"}
    }
    ...
    if not load_pickle or is_debug:
        ...
        df["timediff-elapsedtime_bin300"] = [x//1000+150 if x//1000+150 < 300 else 300 for x in df["timediff-elapsedtime"]]
        df = df[["user_id", "content_id", "content_type_id", "part", "user_answer", "answered_correctly",
                 ...
                 "timediff-elapsedtime_bin300"]]
        print(df.head(10))
        print(df["timediff-elapsedtime_bin300"].value_counts())
    ...
    os.makedirs("../input/feature_engineering/model211", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model211/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model211/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model211/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model211/val.pickle", "rb") as f:
