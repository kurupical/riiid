...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        label = np.zeros(self.max_seq, dtype=int) - 1
        ...
        # decoder
        content_id_decoder = np.zeros(self.max_seq, dtype=int)
        content_id_decoder[1:] = q[:-1]

        elapsed_time_out = np.zeros(self.max_seq, dtype=int)
        elapsed_time_out[1:] = elapsed_time[1:]

        label_decoder = np.zeros(self.max_seq, dtype=int)
        label_decoder[1:] = qa[:-1] + 2  # 1: lecture 2: not correct 3: correct

        prior_q_out = np.zeros(self.max_seq, dtype=int)
        prior_q_out[1:] = prior_q[1:] # 0: nan, 1: lecture 2: False 3: True

        ua_out = np.zeros(self.max_seq, dtype=int)
        ua_out[1:] = ua[:-1] + 1 # 0: nan, 1: lecture, 2~5: answer

        return {
            # encoder
            "content_id_encoder": content_id_encoder,
            "timedelta": duration_previous_content,
            "container_id": container_id,
            "part": part,
            "rate_diff": rate_diff,
            # decoder
            "content_id_decoder": content_id_decoder,
            "user_answer": ua_out,
            "prior_q": prior_q_out,
            "elapsed_time": elapsed_time_out,
...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
                 emb1=256, emb2=32, n_encoder_layer=1, n_decoder_layer=1):
        super(SAKTModel, self).__init__()
        self.n_skill = n_skill
        self.embed_dim = embed_dim
        embed_dim = emb1 + emb2*4

        # common
        self.content_id_embedding = nn.Embedding(n_skill+1, emb1)

        # encoder
        encoder_cat_dim = emb1 + emb2*3
        self.timedelta_embedding = nn.Embedding(302, emb2)
        self.container_id_embedding = nn.Embedding(302, emb2)
        self.part_embedding = nn.Embedding(8, emb2)
        self.rate_diff_embedding = ContEmbedding(input_dim=1, embed_dim=emb2, seq_len=max_seq)
        self.cat_enc_embedding = nn.Sequential(
            nn.Linear(encoder_cat_dim, encoder_cat_dim),
            nn.LayerNorm(encoder_cat_dim)
        )

        # decoder
        decoder_cat_dim = emb1 + emb2*4
        self.user_answer_embedding = nn.Embedding(6, emb2)
        self.prior_q_embedding = nn.Embedding(4, emb2)
        self.elapsed_time_embedding = nn.Embedding(302, emb2)
        self.label_decoder = nn.Embedding(4, emb2)
        self.cat_dec_embedding = nn.Sequential(
            nn.Linear(decoder_cat_dim, decoder_cat_dim),
            nn.LayerNorm(decoder_cat_dim)
        )

        # all_embedding
        encoder_layer = nn.TransformerEncoderLayer(d_model=embed_dim*2, nhead=num_heads, dropout=dropout)
        self.transformer_enc = nn.TransformerEncoder(encoder_layer=encoder_layer, num_layers=1)

        self.gru = nn.GRU(input_size=embed_dim*2, hidden_size=embed_dim*2)

        self.layer_normal = nn.LayerNorm(embed_dim*2)

        self.ffn = FFN(embed_dim*2)
    ...
    def forward(self, item, device):
        ...
        # decoder
        # ===============
        content_id_encoder = item["content_id_encoder"].to(device).long()
        timedelta = item["timedelta"].to(device).long()
        container_id = item["container_id"].to(device).long()
        part = item["part"].to(device).long()
        rate_diff = item["rate_diff"].to(device).float()

        cat_dec_emb = torch.cat([
            self.content_id_embedding(content_id_encoder),
            self.timedelta_embedding(timedelta),
            self.container_id_embedding(container_id),
            self.part_embedding(part)
        ], dim=2)
        cat_dec_emb = self.cat_enc_embedding(cat_dec_emb)
        cont_enc_emb = self.rate_diff_embedding(rate_diff.view(rate_diff.shape[0], rate_diff.shape[1], -1))

        dec = torch.cat([cat_dec_emb, cont_enc_emb], dim=2)  # batch_size

        # ===============
        # encoder
        # ===============
        content_id_decoder = item["content_id_decoder"].to(device).long()
        ...
        label_decoder = item["label_decoder"].to(device).long()

        cat_enc_emb = torch.cat([
            self.content_id_embedding(content_id_decoder),
            self.user_answer_embedding(user_answer),
            self.prior_q_embedding(prior_q),
            self.elapsed_time_embedding(elapsed_time),
            self.label_decoder(label_decoder),
        ], dim=2)
        enc = self.cat_dec_embedding(cat_enc_emb)

        # ===============
        # transformer
        # ===============
        device = content_id_decoder.device
        att_mask = future_mask(content_id_decoder.size(1)).to(device)

        # decoder
        enc = enc.permute(1, 0, 2) # [bs, s_len, embed] => [s_len, bs, embed]
        dec = dec.permute(1, 0, 2) # [bs, s_len, embed] => [s_len, bs, embed]

        data = torch.cat([enc, dec], dim=2)
        att_dec = self.transformer_enc(data, mask=att_mask)
...
def main(params: dict,
    ...
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model137/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model137/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model137/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model137/val.pickle", "rb") as f:
    ...
    model = SAKTModel(n_skill, embed_dim=params["embed_dim"], max_seq=params["max_seq"], dropout=dropout,
                      emb1=params["emb1"], emb2=params["emb2"])
