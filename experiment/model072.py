...
def train_epoch(model, train_iterator, val_iterator, optim, criterion, scheduler, device="cuda"):
    ...
    for item in tqdm(val_iterator):
        ...
        output = model(x, target_id, part, elapsed_time, duration_previous_content, prior_question_had_explanation, user_answer)
...
def main(params: dict,
    ...
    os.makedirs("../input/feature_engineering/model072", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model072/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model072/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model072/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model072/val.pickle", "rb") as f:
