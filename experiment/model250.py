...
class SAKTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return {
            ...
            "content_type_id": get_data(key="content_type_id"),
...
class TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=256, dropout=0.1, activation="relu"):
        ...
        self.lit_layer = LITLayer(input_dim=d_model, embed_dim=d_model, dropout=dropout, activation=self.activation)
...
class SAKTModel(nn.Module):
    def __init__(self, n_skill, max_seq=100, embed_dim=128, num_heads=8, dropout=0.2,
        ...
        self.content_type_id_embedding = nn.Embedding(4, embed_dim_small_cat)
    ...
    def forward(self, item, device):
        ...
        content_type_id = item["content_type_id"].to(device).long()
        ...
        x = torch.cat([x, el_time_emb, dur_emb, e, p, prior_q_emb, container_emb,
                       ...
                       timediff_elapsedtime_emb, content_type_id_emb], dim=2)
...
def main(params: dict,
    ...
    os.makedirs("../input/feature_engineering/model250", exist_ok=True)
    if not is_debug and not load_pickle:
        with open(f"../input/feature_engineering/model250/train.pickle", "wb") as f:
            pickle.dump(dataset_train, f)
        with open(f"../input/feature_engineering/model250/val.pickle", "wb") as f:
            pickle.dump(dataset_val, f)

    if not is_debug and load_pickle:
        with open(f"../input/feature_engineering/model250/train.pickle", "rb") as f:
            dataset_train = pickle.load(f)
        with open(f"../input/feature_engineering/model250/val.pickle", "rb") as f:
